...
// src/__tests__/check-frontend-structure.test.ts（规则①—⑥）共用的
// 夹具仓库根 builder——此前是同构逻辑的两份手写副本（建 workspace yaml +
// 规则⑦登记模块文件），每次登记项变更都要两处同改，漏一处即 CI 假红/假绿
...
//
// 夹具只构成守门脚本的**最小绿基线**（登记项本体，规则①—⑤经 arg2
...
import { DEEP_MODULE_BOUNDARIES, FORBIDDEN_UPWARD_IMPORTS } from "./check-frontend-structure.ts";
...
export interface FixtureRepoOptions {
  ...
  yamlGlob?: boolean;
  /** packages/ 下预创建的成员目录名（不写 package.json——规则①删除即变红靶形） */
  memberDirs?: string[];
...
/** 建夹具仓库根：workspace yaml（glob 声明）+ packages/ 目录 + 规则⑦登记模块文件与规则⑥登记目录（按生产登记表迭代创建，登记项自适配）；
