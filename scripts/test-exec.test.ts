...
interface FixtureOverrides {
  ...
  testSh?: string;
  /** e2e.sh 内容；缺省 = 合规 e2e 形态（一条 `cargo nextest run --test e2e_rstest`）。 */
  e2eSh?: string;
  ...
  /** nextest 仓库配置；缺省 = 合规形态（无 default-filter 排除名单）。 */
  nextestConfig?: string;
  /** 为 true 时不写 nextest 仓库配置（调度配置缺失夹具）。 */
  withoutNextestConfig?: boolean;
  /** 为 true 时删除两个 lib 目标（用于 --doc 登记失效夹具）。 */
  withoutLib?: boolean;
}

/** e2e 目标的 nextest 入口行（真实 scripts/e2e.sh 同址接线，ticket #1496 / 收口 #1508）。 */
const NEXTEST_ENTRY_LINE = "( cd src-tauri && cargo nextest run --workspace --test e2e_rstest )";

/** nextest 仓库配置的合规形态（收口 #1508 后无 default-filter）。 */
...
/** e2e 入口脚本的合规形态（scripts/e2e.sh，ticket #1496 / 收口 #1508）。 */
const DEFAULT_E2E_SH = ["#!/bin/sh", "set -eu", NEXTEST_ENTRY_LINE, ""].join("\n");
...
function makeFixture(overrides: FixtureOverrides = {}): string {
  ...
  if (overrides.withoutNextestConfig !== true) {
    writeFileSync(
...
describe("测试执行器两入口覆盖守门（issue #1112）", () => {
  it("合规夹具：目标清单与两入口并集全等 → 通过", () => {
    ...
    // 6 = 根包 lib + doc、alpha lib + doc、alpha 集成 api / e2e_rstest（e2e nextest）。
    expect(r.output).toContain("目标 6 个");
    expect(r.output).toContain("并发入口 3");
    expect(r.output).toContain("集成测试 1");
    expect(r.output).toContain("e2e(nextest) 入口 1（e2e_rstest，进程级 per-test）");
  });
  ...
  it("同包内 lib 与集成 target 同名不撞键（执行面核对键含 kind）", () => {
    ...
    expect(r.status).toBe(0);
    // 7 = 根包 lib + doc、alpha lib + doc、alpha 集成 api / alpha / e2e_rstest。
    expect(r.output).toContain("目标 7 个");
    expect(r.output).toContain("lib 单测 2");
    expect(r.output).toContain("集成测试 2");
  });

  it("任意 `[[test]] harness = false` → 红（自定义 harness 已随收口 #1508 退役）", () => {
    const r = run([
      "check",
      "--root",
      makeFixture({
    ...
    expect(r.output).toContain("alpha 的 [[test]] custom 声明 harness = false");
    expect(r.output).toContain("退役");
  });

  it("e2e 入口无 `cargo nextest run … --test <name>` 命令 → 红（不静默降级回并发入口）", () => {
    // 收口 #1508 后 e2e 没有第二条轨道：e2e.sh 必须有一条 nextest 运行命令，删掉即红
    //（不静默降级回并发入口——那会退回进程内单线程串行，正是 ticket #1496 要消灭的形态）。
    const r = run([
      "check",
      "--root",
      makeFixture({ e2eSh: ["#!/bin/sh", "set -eu", ""].join("\n") }),
    ]);
    expect(r.status).toBe(1);
    expect(r.output).toContain("e2e 未由 nextest 进程级 per-test 调度");
  });

  it("e2e 入口 nextest 指向不存在的目标（拼写漂移）→ 红", () => {
    const r = run([
      "check",
      "--root",
      makeFixture({
    ...
    expect(r.output).toContain("工作区没有该");
  });

  it("e2e 入口退回 cargo 口径（`cargo test --test e2e_rstest`）→ 红", () => {
    const r = run([
      "check",
      "--root",
      makeFixture({
        e2eSh: [
          "#!/bin/sh",
          "set -eu",
          "( cd src-tauri && cargo test --workspace --test e2e_rstest )",
          "",
        ].join("\n"),
      }),
    ]);
  ...
  it("nextest 配置出现 default-filter → 红（排除名单会把标准目标静默移出）", () => {
    const r = run([
      "check",
      "--root",
      makeFixture({
    ...
    expect(r.status).toBe(1);
    expect(r.output).toContain("出现 `default-filter`");
    expect(r.output).toContain("nextest 调度面");
  });

  it("nextest 仓库配置缺失 → 红（调度配置单点不存在）", () => {
    const r = run(["check", "--root", makeFixture({ withoutNextestConfig: true })]);
    expect(r.status).toBe(1);
    expect(r.output).toContain("nextest 仓库配置不存在");
  });
  ...
  it("工作区无 doc-test 目标却在 doc 入口登记 --doc → 红（登记失效）", () => {
    const r = run(["check", "--root", makeFixture({ withoutLib: true })]);
    expect(r.status).toBe(1);
  ...
  it("CI 测试执行面：未调用统一 e2e 入口（缺 `../scripts/e2e.sh`）→ 红（CI 侧 e2e 未接入）", () => {
    const r = run([
      "check",
      "--root",
      makeFixture({
    ...
    expect(r.status).toBe(1);
    expect(r.output).toContain("未调用统一 e2e 入口脚本");
  });
