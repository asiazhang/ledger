...
export const INFRA_MODULES: readonly WhitelistEntry[] = [
  ...
  { path: 'db', layer: '基础设施', note: '数据库连接与 schema 守卫（#1127 起 mod.rs 只留声明与再导出，按职责分 migrate / connection / runtime 三文件，facade 为 #1408 新增的异步 DB 门面：写读两线程 + 作业通道 + panic 回滚，ADR-0125；时间与身份工厂自 #1128 升顶层 ids、引导层五模块自 #1131 升顶层 boot，原 db 路径经再导出保持）' },
...
export const MARKET_SYNC_MODULES: readonly WhitelistEntry[] = [
  ...
  { path: 'fund_price_refresh.rs', layer: '域目录', note: '基金现价刷新单元（issue #1377 / #1388 自 fund_nav 拆出）：服务标的信息同步的逐只编排——批量面命中零请求（判定 + 直落库）、未命中退逐只短窗封顶 2 页、无历史序列者一个月短窗；FundSyncStats 统计' },
