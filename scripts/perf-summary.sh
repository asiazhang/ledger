#!/bin/sh
# perf-summary.sh —— 把 ledger-perf bench / bench-import / bench-sync /
# bench-market 的人读 stdout 报告转成 GitHub Job Summary 的 Markdown 片段
# （issue #462；bench-import 接入 issue #532；bench-sync 接入 issue #1628；
# bench-market 接入 issue #1629）。
# 阈值判定由工具自身承担（bench 的 --max-p95-ms，issue #493 / ADR-0068，
# exit code 表达结果；bench-import / bench-sync / bench-market 无门禁、纯
# 观测）；本脚本仍只做格式转换，不做任何阈值判定。
#
# 用法：perf-summary.sh <report.txt> [原始报告折叠标题]
#   <report.txt> 是 `ledger-perf bench` / `bench-import` / `bench-sync` /
#   `bench-market` 的 stdout 报告文件（CI 里由 `… | tee …-report.txt` 落盘）。
#   四者报告行格式同构，本脚本通用。
#   [原始报告折叠标题] 缺省 "ledger-perf bench stdout"。
# 输出：Markdown（基准表格 + 折叠的原始报告）写到 stdout，由调用方重定向进
# $GITHUB_STEP_SUMMARY。
#
# 解析口径（真源 = bench.rs / bench_import.rs / bench_sync.rs /
# bench_market.rs 的 print_report 行格式，四者同构）：
