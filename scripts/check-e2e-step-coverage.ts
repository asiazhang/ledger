...
function collectPatterns(stepFiles: string[], problems: string[]): StepPattern[] {
  ...
  for (const file of stepFiles) {
    ...
    for (const m of masked.matchAll(/#\[(given|when|then)\(\s*regex\s*=\s*"((?:[^"\\]|\\.)*)"\s*\)\]/g)) {
      problems.push(
...
function placeholderMatcher(token: string, typed: string, file: string, line: number, problems: string[]): string {
...
function typedMatcher(token: string, typed: string, file: string, line: number, problems: string[]): string {
  switch (typed) {
    case 'string':
      return '"[^"]*"'
    case 'f32':
...
export function checkCoverage(srcTauriDir: string): {
  ...
  if (!existsSync(testsDir)) {
    return { findings, problems: [`tests 目录不存在：${testsDir}`], targets: [], stats: { features: 0, steps: 0, patterns: 0 } }
  }
  ...
  for (const target of targets) {
    ...
    const runnerPatterns = target.patterns.filter((pattern) => target.runners.includes(pattern.runner))
    ...
    for (const feature of target.boundFeatures) {
      ...
      for (const step of collectFeatureSteps(feature, problems)) {
        ...
        const hits = matchers.filter(
          (candidate) => candidate.pattern.keyword === step.keyword && candidate.regex.test(step.text),
        )
