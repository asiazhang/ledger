#!/bin/sh
# 一键质量检查：前端类型检查 + 守门脚本与测试类型检查 + 前端 oxlint lint（--deny-warnings）+ Rust clippy + Rust fmt 检查 + 文档一致性检查 + 命令注册一致性检查 + 结构守门检查 + i18n key 全等检查 + 弹窗表单行距节奏守门（issue #804 / ADR-0079 决策 4）+ 测试桩守门检查（参考数据手搓桩 + 同回调重复桩）+ Rust 测试守门检查（禁直连建库 + 禁夹具裸 SQL + 禁默认时刻字面量，纯禁令，issue #752 落地 / #758 收口 / ADR-0084）
# 任一环节失败即退出（CI 可直接调用）
set -eu
cd "$(dirname "$0")/.."
