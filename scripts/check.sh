#!/bin/sh
# 一键质量检查：前端类型检查 + 守门脚本与测试类型检查 + 前端 oxlint lint（--deny-warnings）+ 前端 oxfmt 格式检查（--check，issue #1519 / ADR-0128）+ Rust clippy + Rust gate-off 编译检查（infra 默认 feature 不含 axum，issue #1133）+ Rust fmt 检查 + 基础设施 rustdoc 门禁（issue #1139）+ 文档一致性检查 + 命令注册一致性检查 + 结构守门检查 + 前端 workspace 结构守门（成员登记、依赖方向、引用形态、深导入，issue #1149）+ 基础设施账本数据表 DML 禁令（issue #1135 / ADR-0111）+ 后台服务成对拉起守门（issue #961）+ i18n key 全等检查 + 弹窗表单行距节奏守门（issue #804 / ADR-0079 决策 4）+ 测试桩守门检查（参考数据手搓桩 + 同回调重复桩）+ Rust 测试守门检查（禁直连建库 + 禁夹具裸 SQL + 禁默认时刻字面量，纯禁令，issue #752 落地 / #758 收口 / ADR-0084）+ 测试执行覆盖守门（并发入口与非并发入口的覆盖范围全等、新增测试目标不静默漏跑，issue #1112）+ e2e 步骤库覆盖守门（feature 步骤行 ↔ 目标注册面全等 + 每个 feature 至少被一个 e2e 目标绑定，issue #1510 / spec #1494）+ 前端异步守门检查（手搓竞态序号硬零容忍 + catch 直弹 toast 基线冻结，issue #1039 / #1008 决议 4）
...
echo "▶ e2e 步骤库覆盖守门 (bun scripts/check-e2e-step-coverage.ts)"
bun scripts/check-e2e-step-coverage.ts
