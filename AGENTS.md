...
- **界面文案与 i18n（ADR-0048）**：新增用户可见文案一律写入 `src/i18n/locales/<locale>/<域>.json` 并经 `@/i18n` 的 `t()` 引用，不再新增硬编码文案；语言判定链与切换收口在 `src/i18n/`，偏好为轻量设置项；各 locale key 集合全等由 `node scripts/check-i18n-keys.js` 拦截（挂 check.sh 与 CI，双向孤儿非零退出）。刻意不随语言：拼音排序/搜索、周起始日（周一）、ISO 日期格式、输入解析、导出/备份内容。
- **错误码（ADR-0049）**：新增用户可见错误条件用码化构造器（`AppError::coded` / `codedp` / `coded_not_found`，码 `<域>.<条件>` kebab-case 全仓唯一），中文 message 原样保留；码的 zh/en 模板同步补进 `src/i18n/locales/{zh-CN,en-US}/errors.json`（前端按码插值、无码透传），漏翻由 key 全等门槛拦截。序列化只增不改：不手写错误 JSON 拼接、不改 `kind`/`message` 形态。
