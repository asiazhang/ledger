...
- 多语句读闭包（同屏口径跨语句：总量=分量和、分子分母同时点、页与总数、读数↔汇率折算）经 `ensure_transaction` 收进同一读事务（#1699，策略陈述住 `tx_scope` 模块文档）；新增此类读闭包同此纪律，验收带读快照探针测试（删除接线即红），已接线闭集与同根因残留候选核对清单见 #1699 交付报告。
