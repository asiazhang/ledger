...
- **后端分层与归位（壳 / 域 / 基础设施 / crate 内部组织，含 triage 判定）**：读 ADR-0112、ADR-0113、ADR-0111；根包目录白名单见 `scripts/check-structure.ts`。
...
- **依赖升级、工具链 bump 或发布打 tag**：升级读 `docs/agents/dependency-upgrades.md`；发布用 `/run-release`，版本元数据与 `CHANGELOG.md` 由该流程更新。
- **测试改动或脚本/质量检查**：测试读 `docs/contexts/CONTEXT-testing.md`；脚本改动先读目标脚本头部注释，脚本当前行为是真源。

代码行为与词汇表不一致时，按可验证行为修正词汇表。代码行为与 ADR 冲突时，先显式报告冲突，确认决策后再修改 ADR 或实现。

## 体验取舍

- 用户界面使用感受优先：用户使用时基本感受不到迟滞感。并发、锁、超时、缓存与加载的取舍按此标尺裁定——等待成功优于即刻失败，且不等可感知失败出现就先行评估方向性方案（先例：#1720 维持 5s busy_timeout、#1729 WAL 评估开票）。
...
- 账户余额与净资产读持久化缓存（V017，ADR-0067）：写路径在既有事务内对受影响账户整体重算，禁止增量加减；缓存行缺失报码化错误，不静默回退。
- 多语句读闭包（同屏口径跨语句：总量=分量和、分子分母同时点、页与总数、读数↔汇率折算）经 `ensure_transaction` 收进同一读事务（#1699 / #1702，策略陈述住 `tx_scope` 模块文档）；新增此类读闭包同此纪律，验收带读快照探针测试（删除接线即红），已接线闭集与同根因残留候选核对清单见 #1699 / #1702 交付报告。
...
- Rust 侧测试三层各有权威：域行为归域单测、壳行为归 API 集成测试、跨模块用户旅程归 e2e BDD；前端逻辑补 Vitest，BDD world 只保存跨步骤读写的状态。断言归属判据、壳三件套与接线证明见 `docs/contexts/CONTEXT-testing.md`。
...
1. 受影响域、入口与分层已识别，改动落在正确接缝；适用测试、文案/错误翻译与文档同步已完成。
2. 代码或脚本改动已运行 `./scripts/check.sh`；仅文档改动已运行 `./scripts/check-docs.sh`。
3. 未解决的 ADR 冲突、无法运行的检查或未提交的改动已明确报告。
