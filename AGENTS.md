...
目标依赖方向是 **壳 → 域 → 基础设施**，域不依赖壳。IPC 壳（`src-tauri/src/commands/`）与 HTTP 壳（`src-tauri/src/api_server.rs`）负责参数解包、事务边界和信号发射，不含业务语义；业务语义进入以域命名的顶层域目录；无域语义的数据库、信号、模型、错误和设置能力进入基础设施。新代码不得扩大壳层业务语义。
