...
- **用户可见缺陷**（数据错误、功能失效、安全风险）：直接修掉，每个缺陷单独提交、提交信息写明根因，交付报告显式标注「范围外修复」并列出证据；连续发现多个时照常逐个处理、一次性报告。
- **设计或范围问题**（决定该不该改、要不要顺带重构）：停下报告，等确认后再动。
- **既有技术债**：单独立 GitHub issue，交付报告列出编号，不顺手修。

判据：修掉后行为是否变得更正确——是则按用户可见缺陷处理，只是更整齐则按技术债立单。
