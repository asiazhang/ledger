...
目标依赖方向是 **壳 → 域 → 基础设施**，域不依赖壳。IPC 壳（`src-tauri/src/commands/`）与 HTTP 壳（`src-tauri/src/api_server.rs`）负责参数解包、事务边界和信号发射，不含业务语义；业务语义进入以域命名的顶层域目录；无域语义的数据库、信号、错误和设置能力进入基础设施（模型已随域归位，ADR-0059）。新代码不得扩大壳层业务语义。
...
- 调用 `/implement` 实施代码改动时使用独立 git worktree，并在工作树内完成验证和提交；只读审查不修改、不提交；研究任务是否写入文档，以用户要求和对应 skill 为准。worktree 缺少前端依赖时先运行 `pnpm install`。
