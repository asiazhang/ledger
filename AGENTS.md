...
- **域目录** = 顶层按域组织的引擎实现，已定格：`src-tauri/src/transaction/`（核心交易）、`src-tauri/src/scheduled_transactions/`（定时计划）、`src-tauri/src/item/`（物品，含域 API `item::domain` 七项短名接口；#397 阶段 1 归位）。域行为代码一律进域目录；未归位域随逐域搬迁票移动，commands 目录内不再新增业务实现。
