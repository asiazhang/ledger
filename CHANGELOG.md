...
- **投资**：「同步标的信息」新增确定进度条——触发后即时显示蓝色细条与「同步标的信息 37/100」计数，由后端逐标的进度事件驱动，被跳过的无通道行不计入分母、「已是最新」照常推进；盈亏页当前持仓卡与标的页标的列表两处就近展示、行为一致；完成后进度条收起并照旧显示「已同步 N 只，跳过 M 只」，失败即收起并提示错误，同步期间界面照常可用（[#897]）。
- **账本**：新增多账本（Book）——侧栏左下角常驻账本入口，弹层内可新建、改名、移除（仅摘登记，文件保留在磁盘）与切换账本；切换经轻量确认后应用原地重载进入目标账本，加密账本切换后落解锁屏。现有数据自动成为唯一的「默认账本」，升级无感（[#831]、[#834]）。
- **账本**：多账本外壳适配——自动/手动备份产物命名携带账本标识，共享备份目录内各账本独立滚动清理、互不覆盖；自动解锁缓存按账本独立存取与清除，切到无缓存的加密账本仍要求手输主口令；「更改数据位置」收窄为搬移当前活动账本，其他账本原地不动，搬迁未完成前不可变更账本登记（[#836]）。
- **多端同步（基座）**：记账动作开始留下可同步的操作日志（OpLog）——桌面端每次创建/修改/删除交易，都会把这条动作连同折算结果写入本机日志，重复送达不会记两次；新库新增同步元数据表（设备标识、端内逻辑时钟），每台设备首次使用自动生成设备标识（[#855]）。
- **多端同步（合并语义）**：双端并发改动开始可安全合并——两台设备同时改同一笔账时按确定性时间线取在后的一方，被覆盖的改动仍留日志可查；两台设备同时自动执行同一期定时计划只记一次账（期次幂等键防双扣）；执行不了的同步操作（如往已删账户记账、更高版本应用产生的操作）进入挂起清单并明确提示，不再中断其余同步、也不静默丢弃（[#856]）。
- **多端同步（检查点）**：任意时刻可产出检查点快照（整库全量快照 + 各来源流的已应用位点），新设备加入凭快照引导、只重放位点之后的增量即达一致状态，无需从头回放日志；位点停在未应用（挂起）操作之前，重投递自愈；日志截断机制就位（含不丢挂起操作等安全约束）但默认不启用（[#857]）。
- **多端同步（通道）**：数据开始真正在设备之间流动——同步通道抽象落地并内置 WebDAV 后端，桌面与移动端走同一代码路径，网盘目录按「每账本一世界、每设备一流、分段上云」组织；同步包以加密信封按文件整体上云，密钥由主口令派生、口令不随包走，未开加密模式时允许明文同步（同步状态显著提示）；凭据错误、网络失败给出明确可重试的错误，本地记账不受影响；检查点可发布上云、新端凭其引导（[#859]）。
- **多端同步（账本级设置）**：本位币基准可在设置页「分类」页签配置——修改后随多端同步在所有设备一致生效，并发修改按确定性规则取序末者、不产生折算口径分叉；原「通用」页签的「默认币种」更名为「展示币种」，仍只影响本设备的金额展示格式与新表单预选币种，不随同步（[#858]）。
- **移动端**：窗口宽度 <840 时切换移动档导航壳——顶部应用栏（当前视图名 + 汉堡按钮）+ 导航抽屉，完整映射侧栏分组、组内收纳「更多」页与固定项；抽屉开合纳入弹层抑制，导航即关闭（[#842]）。
- **移动端**：触屏设备（含平板横屏）交互替换——交易行新增常显「⋯」按钮（点开与右键同一行菜单）；触控轴下侧栏不再显示快捷键提示、记一笔裸键不再绑定监听；交易表金额点按查看全文、报表分类构成占比点按显示、来源列类型全称常驻展示，触屏悬停信息一击可达（[#843]）。
...
- **投资**：持仓页签明细表与盈亏页签各表排版调整——数量/成本/现价/市值/盈亏等数值列右对齐并启用等宽数字，长名称列弹性伸展、单行截断悬停显示全名，代码/账户/日期等短内容列按内容定宽；宽窗口下不再出现名称折行与大片留白（[#920]）。
- **投资**：盈亏数字着色改为「红涨绿跌」（A股/基金惯例）——持仓页签未实现盈亏与盈亏页签已实现盈亏，盈利显红、亏损显绿，并随亮/暗主题取色（原为固定的绿涨红跌亮色值）（[#920]）。
