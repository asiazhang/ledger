...
- **投资**：货币基金按「价格恒定标的」出新口径——单位净值恒 1.0000，标的列表「价格来源」列显示「恒定价格」，走势页直出常量线并附一句口径解释（收益以份额结转体现），持仓页「净值日期」列对它显示为空，价格过期提示不再把货基计入；数据源自报口径确认（建档、同步任一次）即自动打标，确认为恒定标的后现价不再随同步刷新；已落库的历史平坦序列保留不动、走势改由常量合成（[#1450]，ADR-0126）。
