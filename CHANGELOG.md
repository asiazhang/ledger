...
- **投资**：新增「添加投资标的」对话框，成为标的创建唯一入口（[#697]）——市场必选录入通道（沪/深/港/美股/场外基金）：场外基金按 6 位代码即拉净值（原「添加基金」语义不变）、沪/深/港 6 位或 5 位代码走行情接口、美股字母 ticker 遍历三交易所；代码查询命中自动识别类型（行情命中 → 股票、类型特征 → ETF）并回填东财权威名称与最新价；未命中在对话框内手动建档兜底（名称必填、类型限债券/ETF/其他、市场取所选值）；原「新建标的」独立弹窗与「添加基金」独立入口收编退役，标的列表市场筛选不展开美股三交易所（UI 只显「美股」）。
