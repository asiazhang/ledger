...
- **投资**：新增资金加权收益率（XIRR）——持仓页每标的、盈亏页账户级与全账级的年化内部收益率（实际天数 / 365），回答「我的钱实际赚了多少」；与金额口径并列、互不换算，按币种分组不折算，缺价持仓不计入，现金流无解时显式标注无法计算；支持区间选择（区间开始存量持仓按区间首日市值折为期初投入，不改账务）；补记（期初存量）持仓及含其的账户 / 全账合计改给未年化收益率并标注口径（[#1195]，[#1343]，[#1346]，ADR-0115）。
