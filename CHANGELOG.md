...
- **保险**：保单与商户分家换轨（ADR-0082）——新建/编辑保单的保险公司改为从保司字典选择（拼音可搜、输入新名即建保司而非商户，软删保司不可再选但存量保单照常显示）；保单缴费协议不再挂商户，保费流水归保单引用、不再计入商户口径（商户排行回归纯消费口径、商户名搜索不再覆盖保费流水，备注仍可搜）；保单列表保司列改为纯文本（不再跳转商户排行）；保单视角统计（累计已缴/现金流入/下期扣款日）不变（[#713]）。
...
- **投资**：美股端到端打通——股票按代码查询支持纯字母 ticker（遍历纳斯达克/纽交所/美交所三市场候选，返回精确交易所归属与 USD 币种，全不命中 400 中文报错）；标的创建增强对美股 ticker 同路生效，落精确市场与美元、大小写归一幂等（缺省遍历遇东财不可达降级落 unknown）；持仓价格增量同步对美股持仓按精确市场刷价并回填近两年周线，USDCNY 折算走既有汇率候选序列，净资产/可投资资产跨币种口径覆盖美股持仓（[#696]）。
- **投资**：标的创建对 stock 类型开启东财增强（沪深港，与既有 fund 分支对称）：真实代码经东方财富校验并回填权威名称、落最新价现价（导入后持仓立有市值）；查无此码 400 拒绝；东财临时不可达降级建行且保留市场（股票行情通道只依赖市场+代码，降级行价格同步仍可达）；北交所代码与 market 矛盾显式 400。导入知识「投资交易」节改写为「先按代码查询 → 再创建（带精确市场）→ 写 buy/sell」三步法，与「基金申赎」节对称；「名称充代码」收敛为源数据确无代码时的兜底（[#694]）。
- **AI 导入**：新增紧凑契约方言端点 `GET /api/v1/contract`（`ledger-contract-1`）：与标准 OpenAPI 同一契约源的第二机械投影——剥离 OpenAPI 结构样板与出处引用、字段级语义描述原文保留、端点级长教学迁入导入知识对应节（入口提示词契约发现同步改指向新端点）；契约拉取体积 ~46KB → ~17KB（约 14K → 5.1K tokens），带 ≤20KB 体积护栏。标准 `GET /api/v1/openapi.json` 端点/字段/状态码结构零变化、纯增量无破坏（端点级描述按描述政策收敛，ADR-0090，[#839]）。
- **投资**：新增「添加投资标的」对话框，成为标的创建唯一入口（[#697]；六通道修订 [#826]）——市场必选录入通道六选（沪/深/港/美股/场外基金/自定义标的）：场外基金按 6 位代码即拉净值（原「添加基金」语义不变）、沪/深/港 6 位或 5 位代码走行情接口、美股字母 ticker 遍历三交易所；代码查询命中自动识别类型（行情命中 → 股票、类型特征 → ETF）并回填东财权威名称与最新价，查无此码只报错并引导切换自定义标的通道；「自定义标的」通道零查询直接建档（代码自由文本、名称必填、类型限债券/ETF/其他，落库市场未知，同（代码，类型）复用并更新名称）；原「新建标的」独立弹窗与「添加基金」独立入口收编退役，标的列表市场筛选不展开美股三交易所（UI 只显「美股」）。

### BREAKING

- **数据库 schema**：保单表保司字段由商户引用就地换为保司引用（V012 就地修改：`merchant_id` → `insurer_id`，引用 insurers 表，ADR-0082 决策 1/5）。仅全新安装获得新形状；**已执行过旧版迁移的存量库不自动升级，升级到本版本后保单功能不可用，需重建库**（[#713]）。
...
- **投资**：「同步持仓价格」升级为「同步标的信息」并放开覆盖面（[#827]）——标的收集从「当前有持仓」放开为库内全部标的、按通道能力分区（清仓标的与纯建档未交易标的恢复同步，首次同步按既有近两年回填规则补历史；无通道行自动计入跳过统计）；有通道的行同步时以数据源权威名称随行刷新标的名称（行情通道零额外请求，基金通道逐只详情查询；同码自建行名称被数据源覆盖为已接受的产品裁决）；IPC 命令 `sync_holding_prices` 改名 `sync_instrument_info`，按钮文案同步更新（zh/en）。
- **加密/备份**：转换、恢复、搬迁完成后的「应用自动重启」改为原位重引导（进程不退出、界面重载），开发与签名构建行为一致，修复开发构建下重启后白屏（[#644]）。
- **加密**：自动解锁等待有界——钥匙串受阻不再卡在加载态，超时回退手输并提示，手输与恢复入口保持可达（[#644]）。
