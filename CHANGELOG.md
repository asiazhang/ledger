...
- **AI 导入**：新增紧凑契约方言端点 `GET /api/v1/contract`（`ledger-contract-1`）：与标准 OpenAPI 同一契约源的第二机械投影——剥离 OpenAPI 结构样板与出处引用、字段级语义描述原文保留、端点级长教学迁入导入知识对应节（入口提示词契约发现同步改指向新端点）；契约拉取体积 ~46KB → ~17KB（约 14K → 5.1K tokens），带 ≤20KB 体积护栏。标准 `GET /api/v1/openapi.json` 端点/字段/状态码结构零变化、纯增量无破坏（端点级描述按描述政策收敛，ADR-0089，[#839]）。
