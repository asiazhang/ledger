...
- **交易**：非本位币交易写入契约新增逐笔可选显式汇率 `fx_rate`——数据源覆盖不到的日期，调用方（批量导入 / HTTP / IPC）可为该笔直接给定折算用汇率；优先级为「显式 > 序列命中 > 报错」，带显式汇率的行跳过序列查询、折算来源留痕标为「显式」；缺省（不提交）即既有行为零改动，非法值（非正、与本位币同币种携带）报码化错误整笔不落库，且不参与导入幂等身份（同批重跑仍只落一份）（[#1549]）。
- **交易**：非本位币交易随行留痕折算来源——每笔记录本笔折算使用的汇率值与来源（「来自汇率序列」/「来自调用方显式给定」），列表与详情可读回解释「这个本位币金额是怎么来的」；与本位币同币种、不折算（份额调整）及存量行留痕为空（[#1548]，只增不改、存量库免重建）。
- **交易**：编辑非本位币交易时，币种、金额、日期均未变则沿用行内已记录的汇率与本位币金额，不再重查汇率序列——只改备注/分类/商户的历史行不会因数据源当前查不到当年值而改不动；该笔携带显式汇率时以显式为准（显式 > 沿用 > 序列）；任一项变了才按新日期重查重算，查不到仍按既有码化错误失败，不静默沿用旧值（[#1550]）。
- **交易 / HTTP API**：交易列表新增可选参数 `hide_investment_related`（IPC `list_transactions` 与 `GET /api/v1/transactions` 共用，OpenAPI 与 AI 契约自描述面同步）——`true` 时行集与 `total` 排除转出 / 转入 / 出资三端任一端为投资类型账户的行（按账户不按分类：银行卡支付的「投资费用」分类支出照常返回），与既有过滤维度 AND 组合、`total` 与行集同口径；缺省不携带或 `false` 行为完全不变，只增不改（[#1810]，ADR-0136）。
- **交易**：交易页筛选栏新增「隐藏投资相关流水」设备级视图偏好开关——开启后主列表隐藏转出 / 转入 / 出资三端任一端为投资账户的行（充值转账与投资账户直接支付的费用支出一并隐藏；按账户不按分类，银行卡支付的「投资费用」分类支出照常显示），账户 / 商户 / 分类 / 类型 / 日期筛选在收窄后的行集上照常组合，账户下钻不让位；开关状态跨应用启动保持（仅本机设备，不入账本 SQLite、不进备份、不参与多端同步，默认关，脏值自动回默认关）；清除筛选与 ESC 复位只复位筛选、不触达开关；搜索页、投资页「明细」页签与报表 / 预算 / 余额 / 净资产口径不受影响（[#1811]，ADR-0136）。
...
- **投资**：汇率全量回填窗口退役——回填深度不再由「账本最早的非本位币痕迹」派生，全量腿改为整份 ECB 文件灌库（下界 = 数据源起点），深度判据只看落库序列对数据源起点的覆盖；历史投资数据导入不再永久撞「未找到该周汇率：该周历史空缺」（旧判据下历史导入需要的更早痕迹只能由历史写入产生，互相等不到），首次同步自动补齐全部历史周点（[#1759]，ADR-0019 修订记录）。
- **账户**：外币账户余额口径修正——余额缓存此前把现金腿按**本位币折算值**（`amount_native_cents`）累计，与交易行保留的原币金额、账户唯一币种不一致，导致所有外币账户余额（UI 与 `GET /api/v1/accounts/balances` 同源）及净资产腿全错。现改为按现金腿**归属账户的币种**（原币金额）累计，本位币折算只用于净资产等全局口径；纯人民币账本折算值恒等于原币金额，数值不变、无需迁移。存量已错缓存经设置「数据修复」一键审计修复（ADR-0067），或随该账户下次写入自动重算（[#1769]）。
...
[#1550]: https://github.com/asiazhang/ledger/issues/1550
...
[#1759]: https://github.com/asiazhang/ledger/issues/1759
