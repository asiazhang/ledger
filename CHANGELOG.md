...
- **加密**：新增加密模式最小闭环——设置页「数据 → 加密」开启加密并设置主口令，应用把整库一次性转换为加密文件（先产出并验证新库、原子替换、旧库保留明文副本，中途失败原库原样保留），完成后自动重启；启动探测发现密文库后必经解锁屏手输主口令进入应用，口令错误可无限重试且与文件损坏文案可区分；解锁前自动备份调度不运行、AI 导入 HTTP 面返回码化错误；加密库搬迁待解锁后补做、搬迁后仍为加密库；明文库启动路径零改动。「关闭加密」「修改主口令」「本机记住」由后续票交付（[#570]，ADR-0075）。
