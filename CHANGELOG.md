...
- **投资**：货币基金按「价格恒定标的」出新口径——单位净值恒 1.0000，标的列表「价格来源」列显示「恒定价格」，走势页直出常量线并附一句口径解释（收益以份额结转体现），持仓页「净值日期」列对它显示为空，价格过期提示不再把货基计入；数据源自报口径确认（建档、同步任一次）即自动打标，确认为恒定标的后现价不再随同步刷新、同步也不再为它发请求，「同步标的信息」的缺口统计与进度分母一并不含它（计入跳过）；已落库的历史平坦序列保留不动、走势改由常量合成（[#1450]，[#1451]，ADR-0126）。
- **账户**：信用卡账户新增「信用额度」与「账单日 / 还款日」档案——新建与编辑时可填（也可留空事后补填），账户列表的信用卡行显示已用额度百分比（接近额度上限时变色），编辑弹窗显示可用额度、使用率与下次账单日 / 还款日（具体日期）；额度是档案，不进余额、净资产与报表口径（[#1327]，ADR-0119）。
- **投资**：新增资金加权收益率（XIRR）——持仓页每标的、盈亏页账户级与全账级的年化内部收益率（实际天数 / 365），回答「我的钱实际赚了多少」；与金额口径并列、互不换算，按币种分组不折算，缺价持仓不计入，现金流无解时显式标注无法计算；支持区间选择（区间开始存量持仓按区间首日市值折为期初投入，不改账务）；补记（期初存量）持仓改给未年化收益率并标注口径，含仅存量持仓的账户 / 全账合计整项降级（[#1195]，[#1343]，[#1346]，ADR-0115）；存量持仓在有了真实流水后年化恢复，起算点顺延到首笔真实流水、期前存量按该日市值折为期初投入（[#1345]，ADR-0115）。
...
- **投资**：新增现金分红记录——归属某标的的现金收入（到账账户不限投资账户，可直接入银行卡），不摊薄持仓成本、不改份额；投资页「累计收益」口径扩为「持仓收益 + 已实现盈亏 + 累计分红」三腿，与第三方基金平台的同名口径对齐（[#1078]，ADR-0109）。
- **投资**：分红按标的计数、按币种分组进累计收益；分红流水可修改（就地全字段替换）与软删，累计收益实时精确回退（[#1078]）。
- **交易**：交易列表的类型筛选与只读详情新增「分红」类型；分红行只读呈现（归属标的、金额、到账账户、日期），无编辑 / 软删入口（[#1078]，ADR-0109）。
- **AI 导入**：导入知识新增现金分红教学——归属标的解析（同 buy/sell 的按代码三步法）、到账账户与币种、禁止落成普通收入或卖出、累计收益对账与纠错边界、红利再投口径；`dividend` 可用（[#1078]）。
- **AI 导入**：新增投资知识端点 `GET /api/v1/import/knowledge/investment`——投资交易 / 基金申赎 / 基金转换 / 份额调整 / 现金分红五节的教学与对账、纠错口径，两个知识端点均进入契约发现面；投资专属表述（kind 清单、商户排除、读回来源口径、删除级联）只在投资节正面陈述，基础知识只留知识索引与指针（[#1123]、[#1124]，ADR-0110）。
...
- **账本**：新增跨账本投资汇总——入口在账本切换弹层的「全部账本（投资汇总）」，把各本账本的持仓市值、持仓收益、累计收益与可投资资产合并成只读合计；各账本位币一致时直接相加，不一致时按当期汇率折算为当前账本位币并标注口径；加密账本未解锁、尚未初始化或版本不一致的账本逐本显式标注「未计入」，不阻塞其他账本出数；汇总页只读，无任何编辑入口（[#1196]，ADR-0114）。
...
- **多端同步**：内置 S3 兼容对象存储同步通道——用户自备存储桶 + 显式凭据，支持自定义 HTTPS 端点与 path-style 寻址，同步包可选加密上云，检查点可发布上云（[#859]、[#1216]）。
...
- **多端同步**：设置页新增同步卡片——查看上次同步时间与挂起操作数、手动「立即同步」、配置 S3 兼容对象存储通道（端点 / 区域 / 桶 / 前缀 / 密钥 / 寻址方式 + 同步空间，保存前可「测试连接」探测并分层报出凭据 / 目标 / 权限 / 网络问题）；两台桌面设备配置同一同步空间后，手动同步即可保持一致；密文库同步数据以主口令加密上通道，明文库同步时显著提示（[#862]、[#1218]、[#1219]）。
- **多端同步**：同步不再需要惦记手动触发——打开应用即同步、桌面运行期每 10 分钟低频轮询、记完账约 5 秒后上传；挂起操作明细在同步卡片可见（逐条按原因本地化）；密文库同步使用解锁时的口令，不会弹生物认证（[#863]）。
- **多端同步**：Android 手机全面接入——与桌面共享同一套域引擎与界面，配置对象存储通道即可加入同步；存量数据的设备可发布检查点快照，全新空账本的手机经向导从通道引导加入，打开应用即同步，两端互见的账目在各端一致；密文快照引导后手机本库同步转为加密（[#864]）。
- **多端同步**：通道配置提供厂商预设下拉——选中即预填端点、常用地域与寻址方式并给出官方文档链接，加载 / 保存按端点反查回显所选厂商；预设只用于预填、不落库，档位标注区分「已实测 / 未实测」（[#1220]）。
...
- **依赖**：前端引入 oxfmt 格式化器并入质量门——本地与 CI 格式检查（`oxfmt --check`）+ 全仓一次性格式化；开发工具变更，无使用者可见变化（[#1519]）。
- **依赖**：内部依赖维护——前端 vitest 4→5 与 oxlint 升补丁版、Rust 传递依赖刷新至最新 patch，均为开发工具与传递依赖，无使用者可见变化（[#953]）。
- **依赖**：同步信封加密基座由 ring 换为 aws-lc-rs——密钥派生（PBKDF2-HMAC-SHA512）、AEAD（AES-256-GCM）与随机源换实现，信封格式与派生参数不变、无使用者可见变化（[#1215]）。
...
- **备份**：修复自动备份连续失败时无任何用户可见提示——失败本就保留脏标记待下周期重试，但现场结构性持续失败（如备份目录永久失效）时会无限静默；现在连续失败达阈值（3 次，约 30 分钟）后设置页「自动备份」卡片显著提示失败次数与排查指引，一次成功备份后提示自动消失；单次失败不上浮界面，不弹阻塞弹窗（[#1456]）。

- **投资**：修复标的远程搜索与标的列表的在途请求乱序覆盖——快速连续输入、切换市场 / 只看持仓或翻页时，先发请求若晚于后发请求返回，候选或列表会显示成先发查询的结果、与当前输入 / 筛选不符；现在新查询发起即作废在途，迟到旧结果不落位，清空搜索后先前发出的请求也不再填回结果（投资录入表单、盈亏页标的筛选、标的页列表三处）（[#1401]）。
- **实物资产**：修复切换「在持 / 已处置」筛选后列表短暂显示另一筛选的资产——切换恰逢一次列表重拉在途时，新筛选沿用了旧筛选的加载结果，且之后无失效信号就不再自动纠正；现在筛选切换即作废在途重拉并按新筛选立即重拉，迟到旧结果不落位（合计不受影响，后端恒算在持）（[#1381]，ADR-0123 修订注）。
- **投资**：修复货币基金市值少计约七成——货基的现价被按「万份收益」收录（如 0.3117），而货基单位净值恒为 1.0000（收益以份额结转体现）；基金查询/添加与净值同步各东财通道改为按响应自报口径识别货基，现价按 1.0000 落、净值日期取万份收益最新日期，净值曲线不再显示异常下跌；已写错的现价自下次同步起自愈，已回填的错误历史点位不自动重写（[#1342]，ADR-0038 修订）。
- **分类**：修复编辑分类时选「无父分类」静默不生效——清空父分类后保存，界面提示已更新但子分类仍挂在原父下；更新入参现支持「不给即不改、给 `null` 即清空」（[#1327] 范围外修复）。
- **分类**：修复编辑分类时清空图标静默不生效——图标输入框清空后保存，图标原样还在；图标字段现同样支持「给 `null` 即清空」（[#1327] 范围外修复）。
- **投资**：修复已终止（清盘）基金被误报「查无基金代码」——这类基金被东方财富摘出在用索引但档案与历史净值通道仍在服务，基金查询与标的创建改按「搜索索引 ∪ 档案通道」判定存在，可回填权威名称与最后一期净值并正常建行（AI 迁移不再只能降级为其他类型或整行跳过）；同批修复「同步标的信息」遇这类基金时名称刷新中断整次同步（[#1212]，ADR-0039 修订）。
