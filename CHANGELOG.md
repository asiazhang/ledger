...
- **设置**：路径与文件的复制、定位通道——界面文本全局不可选，为路径与文件补齐显式按钮通道：数据存储位置卡的当前生效位置与待生效新位置、最近备份路径新增「复制路径」（复制完整路径并有成功提示，失败有错误提示）；备份文件列表行新增「在访达中显示」，经系统文件管理器直接定位该备份文件（[#653]）。

- **设置**：备份页签新增手动刷新——备份文件列表卡头部新增「刷新」按钮，用户在文件管理器手动增删备份文件后可使列表与磁盘一致，不再依赖应用内操作或信号触发（[#651]）。

- **备份**：备份与恢复跟随加密语义（模式随文件走）——备份元数据新增可选加密标记（备份时探测产物写入，旧备份缺标记视为明文、列表与恢复不报错）；备份列表新增「加密」列，密文备份显示锁形标记；恢复确认升级为应用内弹窗，当前库与备份加密模式不一致时显著警告，密文备份需输入该备份的主口令（口令错误可就地重试）；加密库的备份/自动备份/恢复安全备份产物为密文、跨模式恢复后由启动探测接管实际模式（[#572]，ADR-0075 决策 7）。

- **加密**：新增加密模式最小闭环——设置页「数据 → 加密」开启加密并设置主口令，应用把整库一次性转换为加密文件（先产出并验证新库、原子替换、旧库保留明文副本，中途失败原库原样保留），完成后自动重启；启动探测发现密文库后必经解锁屏手输主口令进入应用，口令错误可无限重试且与文件损坏文案可区分；解锁前自动备份调度不运行、AI 导入 HTTP 面返回码化错误；加密库搬迁待解锁后补做、搬迁后仍为加密库；明文库启动路径零改动。「本机记住」由后续票交付（[#570]，ADR-0075）。
- **加密**：设置页加密卡片新增「关闭加密」与「修改主口令」——与开启加密同一套整库转换机制：关闭加密需输入当前主口令，确认后整库转回明文库，重启后不再出现解锁屏；修改主口令在旧口令验证通过后转入新口令的新库，重启后以新口令解锁、旧口令失效。两形态完成自动重启、失败原库原样保留（不存在半加密状态），旧库按既有保留语义留副本（密文副本仍凭旧口令可开），确认与结果文案经 i18n（[#571]，ADR-0075）。
- **加密**：解锁屏新增常驻「忘记口令」入口——先看无后门后果说明（数据在本应用中不可恢复）再二次确认，确认后重置为全新明文空库并立即回到明文模式，重置后可再次开启加密；旧账本按既有重置命名语义保留为密文副本（`ledger.db.bak`，无口令不可读），日后想起口令数据仍可救回（[#573]，ADR-0075）。
- **加密**：新增「本机记住主口令」——设备本地轻量设置项（前端 localStorage，不落库、不随备份迁移），开启后主口令本身缓存于系统钥匙串（macOS 以 Touch ID 生物认证门保护；密钥仍由口令派生、备份跨设备可移植性不受影响），下次启动有缓存时经生物认证自动解锁、取消或钥匙串无缓存/被清时回退手输；平台不支持时隐藏该选项、回退手输；关闭「记住」、关闭加密或忘记口令重置会清缓存恢复手输（[#574]，ADR-0075 决策 3/5）。
- **加密**：「本机记住主口令」新增开发态回退——开发/未签名构建（`tauri dev` / debug）的本机缓存降级为不带 Touch ID 门的钥匙串条目（仍仅本机可读），启动自动解锁免生物认证直接凭缓存进入，不再因缺少 Apple 证书签名权益而不可用；发布构建维持生物认证门不变；钥匙串访问受构建签名形态限制时报专属错误（不再误报「生物认证未启用」）；设置页在开发回退形态下显示可预期提示，与「平台不支持」明确区分（[#662]）。
...
- **报表**：商户消费排行载荷每行新增「交易笔数」`transaction_count`——该商户在所选期间内、参与排行口径（支出 + 退款）的交易记录数，与金额同口径：退款笔数计入、无商户交易不计数、软删商户历史引用照常计数；TopN 截断与全量合计行为不变（[#617]）。
...
- **备份与数据文件**：解锁屏新增常驻「从备份文件恢复」入口（与「忘记口令」并列）——密文库等待解锁时即可选定备份文件走既有恢复全语义（跨模式警告、恢复前安全备份字节副本、原子替换），恢复成功后应用自动重启进入恢复后的数据；密文备份先用手输过的主口令自动试开、失败弹口令框可无限重输。同批将解锁失败提示统一为「口令错误或文件损坏」合并口径（口令错误与损坏运行期不可区分，不误报损坏、不引导重置，可无限重试）（[#603]，ADR-0075）。
...
- **报表**：商户消费排行从整张柱状图改为表格——列为商户名（可点击下钻直达该商户本期支出与退款明细，替代原点柱入口）/ 金额分布内嵌降序条（条长按名次金额比例、配色沿用分类色板，负净额不画条但金额与占比照实）/ 金额 / 占比 / 交易笔数；金额仍受金额隐私模式掩码，Top 5 / Top 10 档位与空态不变（[#618]）。
- **设置**：备份页签卡片重排与危险视觉修正——卡片顺序调整为 备份（动作）→ 备份目录 → 自动备份 → 备份文件列表 → 恢复，最常用的一键备份免滚动直达；恢复入口按钮降为 default 形态（红色警示由恢复确认弹窗承载，恢复前安全备份＋确认弹窗双闸不变）；恢复卡破坏性警示从低对比度灰字升为显著警示块；「立即清理」按钮升为 warning 次要形态；设置页内容列限宽约 720px、左对齐不居中（[#651]，ADR-0078 决策 4）。
- **加密**：危险确认分级试点（ADR-0078）——新增共享危险确认弹窗封装（error 级红色警示块＋加粗后果句＋红色确认按钮；warning 级琥珀色，后果与兜底说明），开启加密确认迁入共享封装（视觉与语义不变）；修改主口令确认从系统原生对话框升级为 error 级应用内弹窗，承载无后门后果说明（新主口令遗忘即数据不可读），确认后整库转换与自动重启流程不变；主口令设最小长度 8 位（仅前端判定）：开启加密表单与修改主口令的新口令字段对短口令即时红显、提交禁用、不拦截键入，边界值（≥8）可提交（[#650]）。

### Fixed

- **界面**：修复组内「更多」页签右键「移回侧栏」菜单在组满时提示文案溢出菜单容器的问题——提示此前画在菜单外漂浮盖住被收视图内容、行距异常；现菜单项行盒随内容撑开，整颗菜单（菜单项 + 提示）为一块实心浮层，置灰语义与文案零变化（[#647]）。
