...
1. **存量不一致规模 = 0 行。** 真实库 `items` 表 0 行（含软删），物品功能自 v0.4.0 交付以来在真实库与备份目录全部可读库（43 份含 items 表的库，含 zip 内副本）上**从未写入过任何一行**；全库 4,459 笔交易 0 笔跨币种，物品不一致的物质基础（跨币种物品）不存在。回填无可回填对象。
...
对真实库（只读 `sqlite3 -readonly`）与历史备份目录全量条目（zip 解包至临时目录后只读打开）扫描：
...
| 历史备份目录 `backups/` | 261 条目（216 个空 temp 文件、24 个 zip、20 个裸 .db、2 个 json、1 目录等）；**43 份含 items 表的库物品行数全为 0**，1 份 zip（2026-08-23，user_version=4）schema 早于 V009、无 items 表 |
...
【代码/实测】以最新 tag `v0.7.0`（2026-09-19，5f337e71）为界，`v0.7.0..HEAD` 在评估时点共 99 提交（本报告提交后为 100）：
