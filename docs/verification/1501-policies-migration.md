...
- `tests/e2e_rstest.rs`：新增 3 条 `scenarios!` 绑定（policies 11 / policy_agreement 10 /
  ...
  `policy_stats_steps.rs` 与共享步骤父模块 `scheduled_steps.rs`（后者 #1500 已并入，
  合并时去重模块声明）；新增运行时注册表断言
  `policy_steps_are_registered_in_rstest_bdd`（三文件 20 / 11 / 7 条注册与
...
1. **AC1 场景数 = feature 内 Scenario 数（新目标逐 feature 全等）**：
   ...
   本票三份 feature 生成的测试经 `cargo nextest list` 过滤计数各 11 / 10 / 8，
   过滤运行 29 tests / 29 passed。本票验证时新目标全量 81 tests / 81 passed /
   0 skipped（含本票 29 场景 + 1 条运行时注册表断言）；合入 #1500 后全量
   162 tests / 162 passed / 0 skipped。
...
3. **静态覆盖守门**：`bun scripts/check-e2e-step-coverage.ts` → 目标 2 个 · 绑定
   feature 40 个 · **未覆盖 0 · 歧义 0 · 无绑定 0**。本票验证时步骤行 3667 条 ·
   注册 875 条（新目标绑定 feature 8 · 注册 151 · 步骤 528）；合入 #1500 后步骤行
   4075 条 · 注册 963 条（新目标绑定 feature 16 · 注册 239 · 步骤 936）。即被绑
   feature 的每条步骤行在新目标注册面恰好一次命中，删任一注册即红。
...
5. **全量测试**：`./scripts/test.sh` 退出码 0——并发入口 32/32 个二进制
   （1788 passed / 0 failed / 4 ignored），e2e 新目标全绿（本票验证时 81/81，合入
   #1500 后 162/162；含本票 29 个场景），旧目标 40 features / 453 scenarios /
   3139 steps 全绿，doc-test 各包通过。
...
2. **删绑定 → 新目标场景消失**：临时删去 `scenarios!("tests/e2e/features/policies.feature")`
   → 新目标 `cargo nextest list` 减少 11 条（policies 的 11 个场景测试消失；本票验证
   时 81→70，合入 #1500 后同口径 162→151）。
...
`main` 在 PR 评审期间合入 #1500（物品与实物资产域迁入新目标），与本票在
`tests/e2e_rstest.rs` 冲突两处——feature 清单注释（两票各追加自己的域）与
`mod scheduled_steps;` 声明（两票都按需并入同一父模块）。解决口径：

- 注释合并列出两票全部已迁 feature，并保留 #1500 对 #1489 间歇性红的说明；
- 模块声明去重保留一份，注释改为同时点明「#1500 汇率夹具」与「#1501 保单协议
  期次步骤」两个消费者；
- `tests/e2e/scheduled_steps/occurrence.rs` 的注册自动合并：#1500 的
