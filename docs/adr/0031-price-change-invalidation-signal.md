...
> 注（2026-09-12 grilling 复核）：生产者清单以投资域词汇表「价格失效信号」词条为准，现已扩至五处——标的信息同步、手动报价、以及行情接入（QuoteAdoption，ADR-0103）落库半边的三条创建路径（场外基金按代码即拉、场内标的按代码添加、AI/HTTP 标的创建落现价）。本 ADR 决策 2 的「两个同步命令」为历史起点，清单不在此复制。
>
> 注（2026-09-14，issue #1277）：标的信息同步的「实际写入」判定修订为**成败同判**——同步写路径换装分段写入口（issue #1276）后逐段 autocommit，中途失败的运行已落库的写入仍是既成事实（失败 ≠ 未写过），失败收尾同样按实际写入置脏并发信号（写入证据随失败必达，收尾裁决单点在壳层分段写入口）；零写入失败不置脏不广播。决策 2 的触发条件「实际写入」口径不变，变化的只是失败路径不再被排除。
