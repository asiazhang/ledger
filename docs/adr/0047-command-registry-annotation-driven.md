...
- 修订注记（2026-09-18，issue #1471）：决策 4 的 TS 调用面识别面收窄口径——命令名字面量单双引号均识别（模板字面量维持不支持，评审兜底），且匹配前先掩码注释（复用 TS 侧注释掩码 `maskComments`，issue #1481 起上收为共享单源模块）：注释里出现的 invoke 形态不再报「Rust 无此命令」假红，双引号 invoke 的幽灵命令不再静默漏检。属同一门禁的识别面补齐非翻案；Rust 注解腿与 build.rs 的共享扫描规则不变，决策 3 的 fn 形态边界不变。
