...
- 修订注记（2026-09-17，issue #1398）：决策 4 的校验精度边界由「命令名存在性」扩展为「命令名 + 请求腿参数键名」——Rust 数据参数名经 lowerCamelCase 转换（与 tauri-macros 的 heck ToLowerCamelCase 绑定同规则）↔ invoke 实参字面量键双向全等，第二段校验住同一脚本、同一扫描边界，属决策 1 门禁的自然延伸非翻案；响应载荷类型面仍不做（搁置于 #1305）。参数提取只读签名，决策 3 的 fn 形态识别规则与「两处扫描规则同步」边界不变。

## 背景

加一个后端命令要手工把「命令存在」同步到多处清单：Rust 侧 `lib.rs` 的
`invoke_handler(generate_handler![…])` 大清单（实施前 78 项），近半年该文件几乎全部提交都是
「追加一行」；TS 侧 `packages/api/src/index.ts` 调用面逐方法追加。任一侧漏改没有校验拦截——漏注册是
...
4. **TS 侧手写调用面保留 + 双向全等校验**：`packages/api/src/index.ts` 调用面本体不动（经删除检验，
...
- 删 TS 方法、或 TS 调用不存在命令 → 校验均列出孤儿并非零退出；生成注册表与原手工清单逐命令 diff 一致。
