...
- 状态：已接受（grilling 定稿，2026-09-11；同日实施前核查二轮勘误，见文末勘误注记；决策 4(b) 门的载体由 #1591 定案改为声明面判定，见文末修订注记）
- 日期：2026-09-11
- 作者：Ledger 项目
...
## 修订注记（#1591 定案，2026-09-20）：决策 4(b) 门的载体改为声明面判定

决策 4(b)「结构守门严形态」的源码文本扫描退役，改由 crate 边界的两条既有核对承担：

- **未声明依赖**：业务域引用 `ledger_sync_engine::` 而未声明该依赖即编译失败（cargo 强于文本扫描）；
- **可声明但禁止**：业务域声明 `ledger-sync-engine` 生产依赖即红（与分层秩无关，ADR-0112 决策 4 修订注记的「方向禁令」类），且这比原门更严——原门只禁止引用内部模块（`ops` / `engine` / `model`），现禁止该依赖本身（同步协议面已下放 `ledger-sync-protocol`，业务域无任何理由依赖多端同步域）。

门 (a) 双源门与门 (c) 注册完备门不变；决策 1–3、5 与「重放不产本地 op」的规格不变，只换其一的机械载体。判据见 ADR-0056 决策 4 修订注记，逐族退役清单与验收见 #1596。
