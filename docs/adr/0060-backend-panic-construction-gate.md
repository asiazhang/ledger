...
- **豁免可清点**：全库 `rg` 豁免点数量与位置即豁免清单（B 7 + C 1 + A 4 + 宏 18 文件 + build.rs 1），漂移在 review 可见。
