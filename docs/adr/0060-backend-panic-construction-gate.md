...
- **豁免可清点**：全库 `rg` 豁免点数量与位置即豁免清单（B 7 + C 2 + 宏 21 文件 + build.rs 1；A 类 4 处已消除，清点为零），漂移在 review 可见。
