...
- 状态：已接受（决策点「交易对手（Counterparty）」已被 ADR-0028 推翻；「期次手动执行」边界已被 ADR-0042 部分推翻——演进为设备级受控自动执行，失败手动重试等其余决策维持）
- 日期：2026-08-28
- 作者：Ledger 项目
...
> **修订（ADR-0042）**：期次执行已演进为设备级受控自动执行（到期追补）：设备级开关开启时，到期期次（`pending` 且计划日期 ≤ 今天、计划 `active`）逐条自动落账，交易日期回填期次计划日期。本节「失败由用户手动重试、不自动重试」维持：追补尝试失败的单期置为 `failed`、走既有手动重试入口，不被下一轮自动反复重试。开关模型与多设备竞态取舍见 ADR-0042。
...
- ADR-0042：定时计划自动执行（到期追补）与设备级开关（部分推翻本 ADR 的期次手动执行边界）。
