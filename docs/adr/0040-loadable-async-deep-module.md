...
3. **错误展示：默认 toast + error 状态双通道**：默认策略为统一 toast（约 50+ 处多数派行为即事实产品语义）；error 可观察状态同时保留，供视图自选渲染（警示位/空态），「既弹又入警示位」的双通道消费即此形态。「策略单点可换」是物理位置不是机制——策略收口为模块内一处私有函数，换策略 = 改一处，不设注入机制。
   > 注（2026-09-11 修订，issue #1008）：新增**每实例静默 opt-out**——`useLoadable(task, { silent: true })` 时 error 照常置位、toast 不弹。默认策略仍是全局单点（无注入机制不变），opt-out 是实例配置而非策略注入。首个消费者是时间范围快捷选择的期间边界拉取（失败 = 优雅降级为不钳制，弹 toast 是背景型请求的噪音）。
...
5. **竞态语义：后发覆盖先发**：请求序号守卫，终态 = 最后一次发起的结果；迟到的前发结果连同其 loading 收尾一并作废。「进行中禁用按钮」是调用方 UI 决策，不进模块语义。
   > 注（2026-09-11 修订，issue #1008）：新增**作废在途**出口 `invalidate()`——竞态序号推进且 loading 收尾置 false（error 不动），不发起新任务，此后迟到的在途结果按既有竞态语义作废。消费者是搜索视图「清空/重置时不许在途结果落位」；没有这个出口，作废后将无人收尾、loading 会永久卡 true。「在途拒绝重入」（忽略后发）**不**是本模块形态——那是消费方动作语义（账本切换闩锁，成功即重启应用），不为它长接口。
   > 注（2026-09-24 修订，issue #1678）：竞态序号簿记改经共享 module `@ledger/latest-wins` 消费（竞态纪元机制实体化成包、seq 别名随收敛退役）——本决策的后发覆盖先发语义与 `invalidate()` 出口语义一字不变，变的只是序号来源单点化。
6. **迁移**：先行样板批四个 composable（useDashboardOverview、useItemDailyTotal、usePortfolioOverview、useRealizedPnl）改写为 Loadable 之上薄壳，对外接口词汇不变；usePortfolioTrend **不进样板批**——其 lastFetchedKey 去重/forceRefresh 是面板特有缓存语义而非样板，等其视图重构时跟随。视图约 46 处 toast 风格等候选 1（计划清单模块）或候选 5（弹窗编排）落地时顺带迁移，不单独立项冲刺；刻意静默的 fire-and-forget 吞错（约 9 处 `.catch(() => {})`）是合法形态，不收编。**新代码一律走 Loadable**，旧风格对新代码关闭；存量自然替换，不设硬期限。存量缺陷两类（静默失败型、`[object Object]` 型）随迁移自然治愈，是仅允许的可见变化。
   > 注（2026-09-11 修订，issue #1008）：存量手搓竞态/loading 的收编批定为本票——预算视图（try/finally 无 catch 型）、计划清单（顺带删除零消费的 refreshVersion 公开面，「等待重拉完成」由 `await run()` 承担）、账本切换的清单加载半边（switching 闩锁除外，见决策 5 注）、报表三卡（两实例：月度+构成合槽承整页 NSpin，商户卡独立槽承接 TopN 竞态）、搜索视图（含 invalidate 出口）、时间范围快捷选择（静默实例 + 成功才赋值的 SWR 形态）。约 46 处 inline-toast 视图维持「顺带迁移、不立专项」不变（实测 47 处/26 文件，插值参数名已漂移出 msg/message 两派，收编须连 i18n 键清理另立专项）；交易列表维持 ADR-0030 决策 6（模块出信号、视图发起）不变。
