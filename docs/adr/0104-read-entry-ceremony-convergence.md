...
1. 迁移后壳层仍余 9 处手写标准锁行（9 个命令），由守门豁免清单 + 动机注释看守——「读命令必经入口」不是全称命题，是「除显式豁免外」的命题。（issue #1283 起 8 处：预检的 manifest GET 出锁，见决策 6 修订注记。）
...
- 锁失败映射从 60 份副本收敛为 read_entry 1 处 + 9 处显式豁免（issue #1283 起 8 处）；双壳读路径形状一致，与写路径对称，ADR-0056「壳层无业务语义」的肉眼可验程度再进一步。
