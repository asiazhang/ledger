# ADR-0049: 错误本地化契约——AppError 码化只增不改，前端按码插值、无码透传

- 状态：已接受
- 日期：2026-09-02
- 作者：Ledger 项目
- 关联：issue #342（spec 二期）、#353（实施票）；ADR-0048（i18n 架构，本契约是其错误提示部分）；词汇表「错误码」（核心交易域）
...
5. **码表落双语文案资源**：全部码的 zh/en 模板收进 `src/i18n/locales/{zh-CN,en-US}/errors.json`
   ...
   （ADR-0048 决策 6）——「码表覆盖全部用户可见错误条件」由码表与后端码点的对照评审
...
- ADR-0048（i18n 架构与 key 全等门槛；本契约复用其文案资源与校验机制）
