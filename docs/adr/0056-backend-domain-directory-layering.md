...
- `scripts/check-structure.js`：白名单 = 首批三个已归位域目录 + 全部基础设施（均先验证对壳层零依赖）；白名单内出现对壳层的模块路径依赖即红。挂入 `scripts/check.sh` 与 CI，与命令注册一致性检查并列。白名单本身即规格——它是质量门槛而非测试（先例：命令注册双向全等检查、i18n key 全等检查、文档一致性检查）。
...
- **已归位（守门白名单）**：核心交易 `transaction/`、定时计划 `scheduled_transactions/`、物品 `item/`（阶段 1 #397 归位：域 API `item::domain` + 溯源守卫 `item::guard` + 成本口径 `item::cost`，壳层压平为单文件 `commands/item.rs`）；基础设施五处（`db/`、`signals.rs`、`models/`、`error.rs`、`settings.rs`）。
- **前置**：口径修缮（#395，独立于搬迁的先行提交）。
- **待归位**：保单 → 预算 → 商户（阶段 2–4）→ 投资域（阶段 5）。
