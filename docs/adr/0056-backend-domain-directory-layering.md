...
- 状态：已接受（修订：模型目录的定性被 ADR-0059 推翻——模型随域归位、不再列为基础设施；本 ADR 其余判定不变，正文不重写，见 ADR-0059）
- 日期：2026-09-02
- 作者：Ledger 项目
...
| **HTTP 接口服务** | `src-tauri/src/api_server.rs` | 壳层 | `src-tauri/src/api_server/` | 与 `commands/` 并列为外部壳层，单文件 1000+ 行待后续专项目录化重构 | #429 |
