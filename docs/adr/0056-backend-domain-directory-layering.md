...
- **已归位（守门白名单首批）**：核心交易 `transaction/`、定时计划 `scheduled_transactions/`、物品成本口径 `item/`（物品主体仍在 `commands/item/`，随阶段 1 归位）；基础设施五处（`db/`、`signals.rs`、`models/`、`error.rs`、`settings.rs`）。
- **前置**：口径修缮（#395，独立于搬迁的先行提交）。
- **待归位**：物品主体（阶段 1）→ 保单 → 预算 → 商户（阶段 2–4）→ 投资域（阶段 5）。
