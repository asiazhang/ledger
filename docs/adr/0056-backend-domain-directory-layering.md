...
- 状态：已接受（修订：模型目录的定性被 ADR-0059 推翻——模型随域归位、不再列为基础设施；基础设施 crate 的「无域语义」定性被 ADR-0111 细化为可守门的两条——不依赖域 crate、不定义账本数据口径，crate 内部四区与内部分层守门见 ADR-0111；本 ADR 其余判定不变，正文不重写）
- 日期：2026-09-02
- 作者：Ledger 项目
