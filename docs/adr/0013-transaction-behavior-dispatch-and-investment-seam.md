...
>
> 注（ADR-0106 / ADR-0109，2026-09-12 grilling 复核）：决策 2 与决策 4 的「`dividend` / `split` 显式拒绝」「实现完整分红/拆股行为明确暂缓」已随两 ADR 激活失效——kind 闭集自 8 种扩为 9 种（split 见 ADR-0106、dividend 见 ADR-0109），行为层对全部 9 种穷尽分派、不再有「暂不支持」分支。本 ADR 正文中「8 种 kind」「仅处理通用交易类型」「暂不支持」等表述自此读作已过期的历史状态；薄 match、investment 出口三件套、事务边界与 kind 归属等其余决策不变。
