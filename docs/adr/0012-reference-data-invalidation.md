...
- 无 schema 变更、无迁移。
- **修订（#241，2026-08-29）**：`ensureFresh()` 与新鲜度窗口（决策 6 提及的动作之一）自落地起零生产消费，已从 store 接口移除；事件驱动失效（`ledger:changed`）是唯一失效机制，无 pull 侧兜底（窗口命中、挂载期兜底拉取均已不存在），需要强制重拉时显式 `refresh()`。决策 1–7 正文保留原貌，本注记为准。
