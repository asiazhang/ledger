...
## 修订（issue #845，ADR-0088 决策 7 落地）

系统返回桥接要求注册表提供「关最上层」出口，触发两处机制演进。**本 ADR 决策不变**（显式上报、不做 DOM 推断、封装收口），演进均在决策框架内：

1. **注册表 Set → 有序栈 + 关闭请求通道**：token 可携带 `requestClose`（封装组件提供），新增 `closeTopOverlay()` 只调用栈顶（打开序 = z 序，后开者在顶）；无通道 token 只参与「有弹层」判定。消费方（useSystemBack）以 `hasOpenOverlay()` 先行判定返回键归属——栈顶不可关时吞掉返回键，不回退路由。
2. **封装根上显式绑定 `:show="resolvedShow"`，终结「一律 attrs 透传」的接线要点**：受控用法绑定值恒等于调用方传入（行为不变）；非受控用法绑定影子态（点击开合经 `update:show` 回写影子态，行为不变）。上文 Boolean 转型陷阱的警示仍然成立且更关键——陷阱在「声明 `show` prop」，模板绑定不经 prop 声明，转型不会发生；上报双通道语义不变。机制收口在 `useOverlayReporting`（8 个 App* 封装共用单点），并顺带把原 AppDrawer 独有的卸载兑底（作用域销毁时撤销上报）扩至全部封装。
