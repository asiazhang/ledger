...
2. **封装根上显式绑定 `:show="resolvedShow"`，终结「一律 attrs 透传」的接线要点**：受控用法绑定值恒等于调用方传入（行为不变）；非受控用法绑定影子态（点击开合经 `update:show` 回写影子态，行为不变）。上文 Boolean 转型陷阱的警示仍然成立且更关键——陷阱在「声明 `show` prop」，模板绑定不经 prop 声明，转型不会发生；上报双通道语义不变。机制收口在 `useOverlayReporting`（8 个 App* 封装共用单点），并顺带把原 AppDrawer 独有的卸载兜底（作用域销毁时撤销上报）扩至全部封装。
