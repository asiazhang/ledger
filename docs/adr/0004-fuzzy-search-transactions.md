...
已废弃（2026-09-17 被 ADR-0027 修订：FTS5 索引及其一致性机制整体移除，改为 Rust 全量扫描 + 统一模糊搜索语义规格；本文保留作历史决策记录，现行语义以 ADR-0027 为准）
...
3. 可搜索内容：交易备注 + 账户名（转账含转出/转入双方账户名）+ 分类名 + 对应拼音首字母。
...
14. **索引刷新策略：后台定时（写入路径零索引工作）**：交易新增/修改/软删除后**不再**由应用层同步重建 FTS 文档，写路径彻底移除索引工作（界面操作零额外开销）；一致性由「触发器纯 SQL 入队 → 后台定时消费」闭环保证：
    - 后台线程（tauri setup 中 `spawn`）固定周期（默认 60 秒）轮询 `search_reindex_queue`，非空则批量消费重建（消费小批量、持锁毫秒级；Daemon 线程随进程退出，无优雅停止）；
    - 批量导入（`create_transactions` / HTTP `/api/v1/transactions/batch`）事务提交后**立即消费一次**：导入是成批写入场景，一次性重建比等下一个周期更合理，且该命令本非 UI 关键路径；
    - 启动对账（文档数 vs 未删除交易数）保留兜底，不一致全量重建；
...
1. 后台刷新线程固定周期（默认 60 秒）检查队列；批量导入命令在事务提交后立即触发一次消费（决策 #14）。
