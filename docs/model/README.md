...
> 实体清单、字段与外键的逐列 ON DELETE 语义以 migration 为唯一事实来源（三类动作的分工见上方「外键约定」）；以下三图只呈现领域结构与关系基数。`v_holdings` 为视图（聚合 security_lots 计算市值与未实现盈亏），不作为实体；`app_settings` 为无外键的 KV 表，不进图，经领域命令读写（ADR-0017）；`sync_device` / `sync_ops` / `sync_parked_ops` 为多端同步元数据（设备标识、逻辑时钟、操作日志、挂起队列），不进图（issue #855 / #856 / ADR-0091）。
