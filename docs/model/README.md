...
> 实体清单、字段与外键的 ON DELETE 语义以 migration 为唯一事实来源；以下三图只呈现领域结构与关系基数。`v_holdings` 为视图（聚合 security_lots 计算市值与未实现盈亏），不作为实体；`app_settings` 为无外键的 KV 表，不进图，经领域命令读写（ADR-0017）。
...
- 「核心表 + 扩展表」的结构决策见 ADR-0003（取代 ADR-0002 的完全独立模型方案）；计划生命周期、周期规则、失败策略等 MVP 行为决策见 ADR-0024。以下两点为 ADR-0024 未覆盖的补充。
- **期次执行的失败语义**：执行失败（如非默认币种缺汇率、CAS 冲突、落库错误）直接返回错误，期次状态保持 pending 不变，重试即再次执行该期次；`failed` 状态为 schema 预留，当前没有写入路径。
- **CAS 并发控制**：基于 version 的乐观锁保证一期在多设备间只会被执行一次——执行设备先抢占（status→processing 且 version+1），仅抢占成功者生成交易并回填 transaction_id；同步时发现 transaction_id 已存在即跳过执行，避免多设备重复扣款或转账。
