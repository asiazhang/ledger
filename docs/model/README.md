...
全库所有跨表引用列均为启用强制的外键（连接统一 `PRAGMA foreign_keys = ON`），删除语义遵循四条约定。逐列动作以 migration 为唯一事实来源；「每个外键都显式声明动作」由迁移审计测试守护（`src-tauri/src/db/tests/migrations.rs`，无白名单），新增迁移漏写会确定性失败。
