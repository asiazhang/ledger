...
- **定义**：后端错误响应中标识错误条件的稳定码（`<域>.<条件>`，全小写 kebab-case，如 `transfer.to-account-required`），与可选插值参数 `params` 一起，供前端按界面语言本地化错误提示（ADR-0049）。
- **边界**：
  - 只增不改契约：错误序列化在既有 `kind`/`message` 之外只增 `code`/`params`，中文 `message` 原样保留、恒为权威；无码或未知码时前端降级透传原文。
  - 每个用户可见错误条件一个码；`Db`/`Parse`/`Io` 系统错误携常通用码（`db.error` 等），底层消息不入码表；码表（zh/en 模板）收在文案资源 errors 域，受 key 全等校验。
