...
- **kind（交易类型，8 种，真源为 `transaction::amount::TransactionKind` 枚举）**：
  ...
  - **行为层编排入口（create / update / delete）**：行为层对外只暴露三个编排入口（连接 + 输入进，见 issue #169）——`revert → plan → 落库 → apply` 的顺序契约、事务边界、守卫文案全部内化为实现细节（`plan / apply` 为模块私有，回退直接委托投资域 `investment::revert`、不另设转发层），调用方不再持有顺序与事务知识。
