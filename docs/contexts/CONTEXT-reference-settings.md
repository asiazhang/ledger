...
- **定义**：只被前端消费、持久化在 localStorage 的设备偏好。判定标准（三条同时满足）：① 读写不需要后端命令参与；② 不随 Backup/Restore 迁移；③ 改动即时生效、只影响本设备（主题、默认币种、备份目录、备份保留上限、自动执行开关等）。与 AppSettings（后端消费或随备份迁移，存 SQLite KV 表）互斥，两套落点不交叉（ADR-0017）。
- **边界**：
