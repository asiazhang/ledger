...
- **定义**：`currencies / accounts / categories / merchants` 四张参考表及其全部派生映射（币种映射、账户映射、分类映射、商户映射、分类树）的统称，作为 UI 字典 / 枚举的**单一来源**，描述"有哪些可选值"。与核心交易域交易流水（`Transaction`，描述"发生了什么"）相对：参考数据量小、变化低频、常驻内存；交易量大、持续追加、按需分页。商户为第四张参考表（ADR-0028）：后端字典表与命令面已落地（见核心交易域 Merchant），前端 store 接入随参考数据失效机制落地。
- **边界**：
  ...
  - **重拉语义（stale-while-revalidate，ADR-0012）**：保留旧数据，成功后才整体替换，界面不闪空。**失效机制唯一是领域规则**：事件驱动（`ledger:changed`）是唯一重拉触发源，无 pull 侧兜底——无新鲜度窗口、无挂载期兜底拉取；需要强制重拉时显式 `refresh()`（在途合并去重）。`status`（`idle | loading | ready | error`）与 `version`（每次成功重拉自增）为失效观测信号（加载状态与重拉次数），不承诺「数据新鲜度可显式感知」。
...
- **边界**：
  - **不改 `initial_balance_cents`**：期初余额保持「建账时锚点」语义不被校准覆盖；调整痕迹在交易列表可见。
  - 调整产生的转账**就是普通 transfer**，不加特殊标记；删除该转账即撤销本次调整。
  - 不计收支统计（transfer 不进 expense_net / income_net）、不影响仪表盘余额合计（黑洞账户排除在外）。
  - 所需币种的黑洞账户缺失时**运行时按需自动创建**（`无(XXX)`、type=`other`、`is_hidden=1`），与种子同形。
...
- **边界**：
  ...
  - 设置页 Tab 分域（ADR-0022；ADR-0034 更名）：通用（轻量设备偏好）→ 分类（参考数据；原「分类与币种」，币种只读展示已移除、字典本体不变）→ 数据（数据文件管理）→ 关于（恒在末位）。
