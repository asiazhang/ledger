...
- **边界**：
  - 间隔固定为 24 小时（ADR-0016），锚点是"距上次备份"而非固定时刻；检查周期短（10 分钟轮询 + 写时顺带检查，ADR-0016 修订注记：原 30 分钟）但备份频率上限是每天一次。
  - 只在应用运行期间生效；系统休眠时调度暂停，唤醒后由短周期轮询在 10 分钟内补上。应用退出时若脏则兜底备份一次（不受每日约束）。
