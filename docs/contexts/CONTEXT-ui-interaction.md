...
- **边界**：
  - **只覆盖实体类下拉**——选项来自前端内存中的实体字典（账户、分类、持仓标的、关联交易等），全量已在前端，拼音过滤不引入任何新数据量要求；实现载体为**薄封装选择器组件**（`src/components/PinyinSelect.vue`，内部包 NSelect，收口 filterable + 拼音 filter，透传 props/attrs/slots），匹配纯函数收口在 `src/utils/pinyin-filter.ts`；范围决策编码在调用点的组件选择上。
  ...
  - 「选项不可全量下发」的远程搜索（如全市场标的检索）属未来独立机制，与本机制互斥并存。
  - 试点（issue #198）落地转账表单转出/转入账户下拉；其余实体下拉按 issue 逐批替换。
