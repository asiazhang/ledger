...
- **边界**：
  - **只覆盖实体类下拉**——选项来自前端内存中的实体字典（账户、分类、持仓标的、关联交易等），全量已在前端，拼音过滤不引入任何新数据量要求；实现载体为薄封装选择器组件（内部包 NSelect，收口 filterable + 拼音 filter，透传 props/slots），范围决策编码在调用点的组件选择上。
  - **枚举类下拉与币种下拉保持裸 NSelect 不加过滤**（issue #195 范围决策）：账户类型、交易类型、订阅周期、市场、年份等选项极少且固定，拼音过滤是无意义交互；币种选项本身含拉丁代码可直接搜，避免「什么都能拼音搜」的心智噪音。
  - 与全局搜索（核心交易域 TransactionSearch）共用同一份语义规格、实现各自独立（前端 pinyin-pro / 后端 pinyin crate，接受生僻多音字的字库级分歧），学一次两处通用。
