...
///
/// `#[doc(hidden)] pub` 而非私有（issue #956 / #1107）：测试支持域的「通道线
/// 格式替身」（`tauri_app_lib::test_support::channel`）据本单点成帧，使测试侧
/// 不再自建第二份摘要口径。这是测试专用放宽（ADR-0084 决策 2 同款：
/// `test_support` 本身即测试专用放宽面），产品路径的消费者仍只有本模块。
#[doc(hidden)]
