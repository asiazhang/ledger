...
///
/// `pub` 而非私有（issue #956；#1107 起 `pub(crate)` 不可满足——消费者
/// `test_support::channel` 在根包，本摘要单点随域迁入本 crate）：测试支持域的
/// 「通道线格式替身」据此成帧，使测试侧不再自建第二份摘要口径。这是测试专用
/// 放宽（ADR-0084 决策 2 同款：`test_support` 本身即测试专用放宽面），产品
/// 路径的消费者仍只有本模块。
