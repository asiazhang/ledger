//! Transport 同步通道（issue #859 / #1221 / ADR-0091 决策 1 修订）：哑字节通道
//! 抽象。
//!
//! 通道只搬运同步文件、不理解载荷、不做任何合并与冲突处理——智能全在各端
//! （ADR-0091 决策 1/2）。两端以同一抽象对接，桌面与移动端共享同一代码路径
//! （域不依赖壳，ADR-0056）；v1 唯一后端是 S3 兼容对象存储（[`s3`]），另有
//! 后端属可逆工程决策（ADR-0091 决策 1 修订：WebDAV 端点在国内主流云厂商普遍
//! 不可得，v1 收敛为对象存储单一后端）。
//!
//! 路径语义：`path` 是相对同步根的逻辑路径（`/` 分隔，如
//! `book-<id>/streams/<device>/seg-...enc`），由通道实现映射到实际地址
//! （对象键 / 文件路径）。文件名集合由 [`super::channel`] 的布局规则产出，
//! 全部为 URL 安全字符。
//!
...
/// 哑字节通道：同步文件搬运的最小操作面。
///
/// 写入恒为整体覆盖（先封包后整体 PUT，不做追加以规避通道弱原子性）；
/// 目录逐级确保存在（无目录概念的后端实现为幂等空操作）。实现须可跨线程共享
/// （同步轮次在后台线程执行，#862 壳层编排）。
pub trait Transport: Send + Sync {
    /// 逐级确保目录存在（幂等：已存在不算错误；无目录概念的后端是空操作）。
...
pub(super) fn validate_logical_path(path: &str) -> ledger_infra::error::Result<()> {
    if path.is_empty()
        || path
            .split('/')
