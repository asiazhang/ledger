...
use tauri_app_lib::test_support::{self, seed_investment_setup};
    ...
    let wire_of_kind = |kind: TransactionKind| {
        ops.iter()
            .filter(|op| match &op.command {
                crate::DomainCommand::Transaction(
...
fn fund_add_by_code_syncs_dictionary_not_quote() {
    ...
    // 价格日期与净值日期同为净值日期。
    let quote = ledger_investment::Quote {
        code: "000001".into(),
        name: "华夏成长混合".into(),
...
fn exchange_rate_and_price_entries_replay_and_converge() {
    ...
    let rate_id = create_exchange_rate(
        &conn_a,
        ledger_currencies::ExchangeRateInput {
            base_code: "EUR".into(),
            quote_code: "CNY".into(),
            rate: 7.8,
    ...
    let _price_id = create_market_price(
        &conn_a,
        ledger_investment::MarketPriceInput {
    ...
    record_manual_price(
        &conn_a,
        &ledger_investment::ManualPriceInput {
            instrument_id: "inst-1".into(),
