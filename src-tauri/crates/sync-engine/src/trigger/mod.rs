...
pub use channel::{
    DEFAULT_SPACE_ID, SyncChannel, SyncChannelConfig, build_channel, configured_channel,
    probe_channel,
};
pub use scheduler::{
...
// `crate` 子树，与 `scheduler` 内的声明一致（不放到 crate 级）。
#[cfg(test)]
pub(in crate::trigger) use scheduler::{drain_write_signals, notify_write_signal};
