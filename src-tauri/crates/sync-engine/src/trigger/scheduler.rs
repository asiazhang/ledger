...
use crate::channel::{ChannelOptions, SyncRoundReport};
...
pub fn sync_on_start<R: Runtime>(app: &AppHandle<R>) {
    ...
    std::thread::spawn(move || {
        let Some(guard) = ledger_backup::lock_conn_with_timeout(&conn) else {
            return;
        };
...
/// （不残留信号触发第二轮）。
pub(in crate::trigger) fn drain_write_signals(
...
/// （不依赖进程级单例，无需测试后门）。
pub(in crate::trigger) fn notify_write_signal(
