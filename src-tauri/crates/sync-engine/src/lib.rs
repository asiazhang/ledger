...
//! 多端同步域 crate（spec #1086 / issue #1107，P4 业务域 crate）：自根包域目录
//! 拆为 workspace 成员 `ledger-sync-engine`，成为在协议 crate 之上、依赖各业务域
//! 的独立编译单元；根包以 `pub use ledger_sync_engine as sync_engine;` 再导出保持
//! 原引用路径零改动。
//!
//! 领域行为（issue #855 / #856 / ADR-0091）：OpLog 基座与双端合并语义——
...
//! 实例 + 根包图内实例）；域单测一律驱动本实例（`crate::…`），建库与共享断言
//! 经 `tauri_app_lib::test_support`（dev-dependency，ADR-0084）。根包图内只有
//! 一份同步域实例，三层测试行为等价证据不受影响。
...
pub use channel::{
    ChannelLayout, ChannelManifest, ChannelOptions, CheckpointPointer, ConnSegment, DirectConn,
    FetchedCheckpoint, RoundConn, SegmentEntry, StreamManifest, SyncRoundReport,
    connection_round_key, fetch_checkpoint, run_round, run_round_with, upload_checkpoint,
