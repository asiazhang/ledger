...
//! 仍是同步 rusqlite 操作。接缝因此是「行情域异步化」与「DB 形态」之间的
//! 稳定边界：编排侧只认本 trait，底层实现可整体更替而编排零分叉——目标态
//! 已落地（生产 = 异步 DB 门面写槽裸作业，issue #1412；#1411 过渡期的阻塞
//! 线程池实现已随桥拆除不复存在）。作业闭包要求 `Send + 'static`（门面作业
//! 形态），编排现场的可变状态（进度回调 / 写入见证 / 通道束）进不了闭包——
//! 需要带回的结果由闭包返回、在 `await` 之后回填编排现场。
//!
