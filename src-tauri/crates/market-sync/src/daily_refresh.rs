...
use super::history::{STARTUP_DELAY, SharedConnSession, WINDOW_POLL_INTERVAL};
...
pub struct DailyPriceRefreshChannelsSlot(pub Arc<Mutex<SyncFetchChannels>>);
        ...
        loop {
            ...
            if !gate.is_locked() && !boot_gate.is_failed() {
                ...
                if last_round_date != Some(today) {
                    ...
                    run_daily_refresh_round(&handle, &conn);
                }
            }
...
/// 速器、前台在途时让行）+ 共享连接会话 + 手动形态同款进度事件 + 写入见证。
/// 失败静默等下一窗口（无用户可报）；实际写入的收尾裁决与手动同步同判——
/// 置脏一次 + 发既有价格失效信号，零写入不置脏不广播。
fn run_daily_refresh_round<R: Runtime>(app: &AppHandle<R>, conn: &Arc<Mutex<Connection>>) {
    let slot = app
...
/// 持束跑一轮：会话（共享连接短段取用）+ 手动形态同款进度事件 + 写入见证。
...
fn run_round_with_channels<R: Runtime>(
    ...
    let result = do_incremental_sync_channels(&session, channels, &mut progress, &mut witness);
