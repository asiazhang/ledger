...
//! - [`source_tail`]：取数尾部契约（spec #1675）——取数单元公共尾部的单一实现：
//!   源畸形时「统一截断 warn（固定「源畸形」文案 + source / error / body 三字段）
//!   → 补降速信号（ADR-0121 决策 5）→ 码化错误映射（ADR-0050）」的顺序由全尾
//!   函数 [`source_tail::finish`] 拥有，五个取数单元与未来的新单元共享；单元只
//!   出解析闭包与错误映射，解码失败与形状判据失败走同一条源畸形路径，降速信号
//!   结构性不可绕过；
...
//! - [`weekly`]：周点落库深原语（spec #1677，词条「周点落库」）——价格历史与
//!   汇率历史共用的周点写入唯一收口：载体中立逐日点集进、是否产生新点出，
//!   周采样 / 判新（同周同值零写入）/ 整周覆盖幂等 / 事务边界四件事收口一处，
//!   旧六种落库形状与「调用方必须包事务」的注释互认退役；
...
mod sina_fund;
/// 取数尾部契约（spec #1675）：源畸形处置顺序（统一截断 warn → 补降速信号 →
/// 码化错误映射）的全尾函数单点，五个取数单元的 fetch 尾部经此退出源畸形路径。
mod source_tail;
mod stock;
...
mod tencent_kline;
/// 周点落库深原语（spec #1677）：价格历史与汇率历史共用的周点写入唯一收口——
/// 周采样、判新（同周同值零写入）、整周覆盖幂等与事务边界（嵌套感知自保障）
/// 吸收进 [`weekly::commit_price_history_weekly`] 与
/// [`weekly::commit_fx_rate_history_weekly`] 两个薄入口 + 私有共享核；置脏与
/// 价格失效信号留调用方编排。
mod weekly;
