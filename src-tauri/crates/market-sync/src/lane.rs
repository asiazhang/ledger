//! 后台车道单轮骨架（issue #1426）：两条后台车道——价格历史补全
//!（[`super::history`]）与每日现价刷新（[`super::daily_refresh`]）——的单轮形态
//! 同构：通道束换装（管理态桩槽优先、生产后台车道束兜底）、门面写槽裸作业会话、
//! 进度发射接线、写入见证、收尾裁决（置脏 + 价格失效信号，成败同判）与失败日志。
//!
//! 骨架的输入 = **编排**（[`LaneRound`] 实现，只回答「这一轮怎么跑」）+ **槽**
...
//!
//! 用户可见语义零变化：两条车道各自的槽类型（`history::BackfillChannelsSlot` /
//! `daily_refresh::DailyPriceRefreshChannelsSlot`）、进度事件名与置脏 / 失效信号
//! 口径逐字保持。
