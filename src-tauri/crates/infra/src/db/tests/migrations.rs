...
/// 当前迁移序列长度（V001–V032，V005 移除不回填，共 31 条）；新增迁移时随
/// `migrations()` 同步更新。钉住「从零迁移到最新」的完整性基线。
const LATEST_SCHEMA_VERSION: usize = 31;
...
fn migration_from_zero_reaches_latest_completely() {
    ...
    for (table, column) in [
        ...
        // V029（issue #1548 / ADR-0011 修订）：折算来源留痕列（使用汇率值 + 来源闭集）。
        ("transactions", "fx_rate_used"),
