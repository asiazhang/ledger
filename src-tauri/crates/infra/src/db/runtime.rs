...
///
/// 门面落地后，调用方的取锁由 DB 线程承担（ADR-0125 决策 1「取用独占收在
/// 门面内」）——命令层与三处统一入口的改道已随 #1410 落地，行情域后台车道
/// 的改道已随 #1412/#1413 落地，生产面已无本形态调用点（ADR-0125 决策 8
/// 收尾注记，issue #1414）；本形态现仅测试消费，本体退役随同步域异步化
///（#1405）另案评估。
