...
//!
//! **多语句读闭包的快照纪律（issue #1699 / #1702）**：rollback journal 下语句之间无快照
//! 隔离，同屏口径（总量=分量和、分子分母同时点、页与总数、读数↔汇率折算）跨多条
//! 语句的读闭包一律经 [`ensure_transaction`] 收进同一读事务——这是读侧的显式策略，
//! 不依赖「窗口足够小」；新增此类读闭包即适用，验收形态是读快照探针测试
//! （`test_support::snapshot_probe`，删除接线即红）。已接线闭集与同根因残留候选
//! 的核对清单随 issue #1699 / #1702 交付报告维护。
//!
