...
impl DbSlotPair {
    ...
    /// 允许住门面与壳层统一入口）。
    ///
    /// 消费方只有壳层分段写入口的 `SegmentLock`（issue #1410 现场判定）：分段
    /// 编排体仍是同步闭包（借用编排现场、非 `'static`），门面作业要求的
    /// `Send + 'static` 形态对它不成立——「每段取连接由门面作业取代」在作用域
    /// 会话接缝异步化（issue #1412：`ScopedSession` 改 async、六处编排 async 化）
    /// 之后才有意义，届时本访问器与分段直锁一并退役。锁仍由本槽的同一把互斥体
    /// 裁定，与门面写线程、换连原语互斥（#1276 的分段语义不变）。
...
impl DbWriteHandle {
    ...
    /// 分段形态的每一段取连接在 issue #1412 前仍是直锁（见 [`Self::write_slot`]），
    /// 届时改用本形态。
