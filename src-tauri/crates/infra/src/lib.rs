...
//! 口径与规则。
//!
//! 边界（ADR-0111 决策 1 / ADR-0112 决策 5）：基础设施→域的生产依赖边为 0——
//! 连接层统一写入口提交点的后置动作（置脏与写时到期检查，ADR-0032）以注册点
//! 反转：注册点住 `db::runtime`，实现由域侧提供、接线在壳层启动（根包 `setup`
//! 与测试建库单点同形）；本 crate 因此不认识任何域。
//!
