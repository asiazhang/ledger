...
//! crate 内私有助手）——此刻改清单会让意图落空、账本目录悬空；意图已完成（目标已有库或双方皆
...
//!（`stable_book_id`，crate 内私有助手；确定性、跨进程稳定），首次登记写入落盘后与随机登记
...
/// 登记账本子目录名（引导解析出的默认目录 `default_dir` 下自动创建的账本目录父目录）。
...
impl BookRegistry {
    ...
    ///（`stable_book_id`，crate 内私有助手）——跨启动稳定，备份命名与钥匙串条目得以按本分域。
