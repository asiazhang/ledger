...
//! 同一口径驱动三处：`list_instruments` 的 `invested` 派生列、标的页「只看持仓」
//! 过滤（`only_invested`）、盈亏页持仓概览。（第四处原为增量同步的标的收集与
//! 统计，自 issue #827 起同步覆盖面放开至库内全部标的、按通道能力分区，
//! 不再消费本谓词。）未来任何「持仓标的」消费方（如定投提醒）只需复用本常量。
//!
