...
use crate::stock::{ResolvedStockCode, derive_quote_currency, resolve_stock_code};
...
fn explicit_market_consistent_with_shape_passes() {
    assert_eq!(
        resolve_stock_code(Some("sh"), "600519").unwrap(),
        resolved("sh", "600519"),
        "显式 market 与形态一致时放行"
    );
    assert_eq!(
        resolve_stock_code(Some("sz"), "000001").unwrap(),
        resolved("sz", "000001")
    );
    assert_eq!(
        resolve_stock_code(Some("hk"), "700").unwrap(),
        resolved("hk", "00700"),
...
fn unsupported_market_outside_closed_set_returns_400() {
    ...
    // 聚合路由值 us 只由解析产物携带，不作为显式入参开放。
    for market in ["bse", "unknown", "us"] {
        let err = resolve_stock_code(Some(market), "600519").unwrap_err();
        assert!(
...
fn beijing_exchange_codes_rejected_as_unsupported() {
    for code in ["430047", "830799", "871981"] {
        ...
        assert!(resolve_stock_code(Some("sh"), code).is_err());
    }
}
