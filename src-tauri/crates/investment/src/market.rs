...
//!
//! 三个类型各答一个问题。[`Market`] 是字符串字面量的唯一住处（`closed_set!`
//! 清单每变体只出现一次）；[`QuoteMarket`] 不携带任何字面量——投影全部骑行
//! [`Market`] 产物；[`StockRoute`] 另带一份路由词汇（`Us` 的拼写 `us` 不是市场
//! 闭集成员，测试断言与日志消费，见其 `as_str` 文档）：
