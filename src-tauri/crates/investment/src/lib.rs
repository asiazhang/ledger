...
//!
//! 依赖方向（spec #1086 / ADR-0112 / issue #1097 AC）：本域消费基础设施、同步
//! 协议、核心交易域、账户域与币种域——核心交易域接缝（`transaction::seams::investment`）
//! 的实现注册面 [`install_transaction_hooks`] 经壳层启动接线（ADR-0112 决策 5，
//! 挂载点⑤）；账户域（AccountType / 余额口径）与币种域（ExchangeRate /
//! ExchangeRateInput，#418 / ADR-0059：投资域是汇率的消费方与录入入口）是两条
//! 域→域合法上层依赖（上层域消费下层域，合法依赖直呼；后者经 #1097 裁决
//! 显性化承认）。行情同步域的查询半边经注入接缝消费（注入签名见 [`fund`] /
//! [`stock`]），对根包（壳层）与行情/多端同步域零直接依赖，反向引用由 cargo
//! 依赖图编译期拒绝（生产依赖面无根包，dev-dependency 环只覆盖测试目标）。
...
/// 同步重放的计划重建接缝（issue #861）：跨 crate 消费——sync_engine::registry
/// 经本接缝执行外来投资命令（与本地写同协议，ADR-0091；再导出面随 crate 拆分
/// pub(crate)→pub，签名与语义不变，ledger-transaction/#1092 同款）。
pub use command::{replay_exchange_rate_command, replay_instrument_command, replay_price_command};
pub use trade::{
