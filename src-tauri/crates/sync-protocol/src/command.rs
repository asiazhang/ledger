...
//! 域命令类型：`DomainCommand` 信封与 15 个域 payload 留在业务域与 sync_engine
...
///
/// 14 个域侧重放入口返回 `Result<()>`，由适配绑定映射为 [`ReplayEffect::Applied`]；
/// 期次触发（OccurrenceKey 独有语义，见 CONTEXT-sync 期次词条）由域侧原样透传
/// [`ReplayEffect::IdempotentHit`]——逼其余 14 个无此概念的域返回它只会让类型说谎，
/// 差异留在适配层比假统一诚实。
