//! 设备标识与逻辑时钟：同步元数据的本机单点（`sync_device` 单行；自
//! sync_engine/device.rs 下放，issue #1089）。
//!
...
/// 互斥（`ledger_infra::db::DbState`）串行化调用，读后写无竞态。
...
/// 分配下一个端内逻辑时钟（+1 并持久化）。
///
/// 必须在本地写事务内调用（[`super::op::record_local`] 保证：先取 DeviceId
/// 再分配时钟，均在 op 产出的同一事务中，随事务提交/回滚）。
pub fn next_clock(conn: &Connection) -> Result<i64> {
    conn.query_row(
...
// 单测：DeviceId 首用生成与持久化、换库新标识、事务回滚不残留——住根包
// `sync_engine/tests/device.rs`（建库两行序经根包 test_support 工厂，ADR-0084
// 规则 1；测试事务的裸 BEGIN/ROLLBACK 是外挂测试目录的豁免形态，ADR-0056 决策 5）。
