//! 金额口径（共享语义区，issue #54 / spec #52）：交易金额口径的单一权威。
//!
//! 职责：[`TransactionKind`] 枚举真源（9 种类型的 DB/wire 小写字符串映射）、
//! kind→度量矩阵（[`signed_amount`] 与四个 SQL 片段 builder 由同一 [`coefficient`]
//! 驱动）、[`convert_to_native`] 本位币折算。不变量：矩阵口径恒一致；折算基准为
//! 全局默认币种（[`default_currency_code`]），不依赖 per-account 币种。ADR 指针：
//! ADR-0113 决策 3.1（本位币接缝契约随消费概念归本共享语义区）。陷阱：`base_currency`
//! 子模块的读取实现由币种域装入、壳层接线，未注册即码化错误。
...
pub fn default_currency_code(conn: &Connection) -> Result<String> {
    self::base_currency::current_base_currency(conn)
}
