...
/// 命令层在落库后另行回写，不属本模块职责。
/// `#[derive(PartialEq)]` 无 `Eq`：`fx_rate` 是 f64（#1549），浮点无 Eq。
...
/// - 本位币折算走 Amount 接缝按交易日入口 [`amount::convert_to_native_on_trade_date`]
///   （#1547：按交易所属 ISO 周命中汇率历史，基准为全局默认币种；当期表只服务读路径；
///   入参显式汇率 > 序列命中 > 报错，#1549）。
///
