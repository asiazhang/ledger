//! 交易读取权威（读路径，列表过滤/排序/分页与单笔读取）。
//!
//! 职责：列表过滤/排序/分页与单笔读取的 SQL 单点。不变量：过滤条件与 total/items
//! 共用同一 WHERE 子句；排序为 date DESC, created_at DESC, id DESC（id 是最终
//! tiebreaker）。ADR 指针：ADR-0113 决策 8 / ADR-0056（域访问器）。陷阱：来源列
//! 与转换扩展由 `read::source` 读时投影填充，接缝注册点在跨域接缝区。
...
use source::{attach_convert_fields, attach_sources};
