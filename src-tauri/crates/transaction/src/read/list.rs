...
pub fn list_transactions_internal(
    ...
    // 读快照一致性（issue #1699）：COUNT、items、来源与转换投影填充是多语句读闭包，
    // rollback journal 下语句之间无快照隔离（写提交落在其间即页与总数错位）——
    // 整体收进同一读事务（嵌套感知：已在事务中则加入外层，回滚权归外层）。
    ensure_transaction(conn, || {
        ...
        let mut sql = format!(
         ...
         to_account_id,funding_account_id,category_id,refund_of_transaction_id,note,date,created_at,updated_at,version,device_id,is_deleted,merchant_id,policy_id,fx_rate_used,fx_rate_source \
