//! 交易×商户接缝（跨域接缝区，issue #1092）：商户名归一化钩子组注册点。
//!
//! 职责：对商户表的查询与建行归商户域实现（`merchants::install_merchant_hooks`），
//! 壳层启动接线；「先查名、校验通过后即建」的两段式时序留写入协议。不变量：两支
//! 钩子一个槽位原子注册（避免「只装查、缺建」半套接线）；未注册即码化错误（携带
//! 商户名的写入显式失败，不静默丢即建）。ADR 指针：ADR-0112 决策 5 / ADR-0113
//! 决策 8。陷阱：失败行不产生碎商户是写入协议语义，不在本模块重演。
