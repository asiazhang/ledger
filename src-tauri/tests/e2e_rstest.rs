...
//!   log_level.feature / sync.feature，#1507）在本目标运行，旧目标行为零变化。
//!   账户 / 交易 / 保单、物品与投资域场景全绿；
...
/// 删掉任一新注册即红。
///
/// 共享文件里的按需注册（`budget_steps.rs` 3 条预算命令步骤、
/// `reports_steps.rs` 2 条月度汇总断言）逐条模式的全等覆盖由静态覆盖守门兜底
/// （ticket #1510）；本断言只担本票整文件转换的八个步骤文件的运行时那半。
#[test]
