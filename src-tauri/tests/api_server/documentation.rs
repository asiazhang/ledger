...
async fn test_import_knowledge_covers_key_conventions() {
    ...
    let required_keywords = [
        ...
        // 出资账户教学关键词锁（issue #939 / ADR-0096 决策 8）：直扣/直付场景
        // 携带出资账户、余额核对含结算账户现金流（含出资账户）、去重哈希纳入
        // 出资账户——新表述被误删或退回旧口径时逐词报红。
        "funding_account_id",
        "出资账户",
        "直扣/直付",
        "现金腿记出资账户",
        ...
        "kind 改成 / 改出",
        "假卖出",
        // 投资 kind 可用性文案（ADR-0109 / issue #1078）：dividend 激活后文案锁
        // 改绿——四类投资 kind 全可用；措辞退回「dividend 不受支持」旧口径时报红。
        "四者均可用",
