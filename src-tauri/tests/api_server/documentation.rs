...
async fn test_import_knowledge_covers_key_conventions() {
    ...
    let required_keywords = [
        ...
        "currency_code",
        "dividend",
        // 跨切面投资语义已收口至投资节（issue #1124），基础侧只留指针：
        // 拆行指针、商户排除指针与对账读回来源口径指针——投资专属规则的
        // 正面陈述退回基础侧时逐词报红。
        "投资流水不落这三类",
        "投资流水的商户规则不在本页陈述",
        "由投资节承载",
        // 知识索引条目内容锁：投资索引条目的一句话语义、行驱动触发、整场
        // 黏住、停止标志与五节名称清单逐一锁定——删除投资索引条目即红
        // （接线型，断言对准 HTTP 响应体内容）。
        "标的解析与投资落账",
        "即须先拉取投资节再落账",
        "整场会话黏住",
...
async fn test_import_investment_knowledge_covers_key_conventions() {
    ...
    let required_keywords = [
        ...
        "不摊薄持仓成本",
        "红利再投",
        // 红利再投口径锁为投资账户内两笔（ADR-0109 2026-09-13 修订）：知识
        // 正文必须显式给出「到账账户 = 投资账户」与两腿「自相抵」——退回
        // 「分红入黑洞账户 + 黑洞出资买入」（丢前者）或退回单条 0 手续费
        // buy（丢后者）时本节逐词报红。本锁只覆盖导入知识投资节正文。
        "到账账户 = 投资账户",
        "自相抵",
        "kinds=dividend",
        ...
        // 字段、标的投影与「入参字段不经读回返回」的措辞逐一锁定。
        "source",
        "entity_id",
        "display_name",
        "转出标的",
        "不经交易读回返回",
        // 投资读回过滤用法：issue #1124 自基础侧「查询纪律」迁入。
        "kinds=buy,sell",
    ];
    for kw in required_keywords {
        assert!(text.contains(kw), "导入知识应包含关键约定关键词 {kw:?}");
...
async fn test_openapi_doc_covers_list_transactions_params_and_schema() {
    ...
    for expected in [
        ...
        "kinds",
        // 视图偏好载荷（issue #1810 / ADR-0136）：只增可选参数
        "hide_investment_related",
        "limit",
        "page",
        "page_size",
    ] {
