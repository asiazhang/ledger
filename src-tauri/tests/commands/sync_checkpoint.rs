...
//! 另钉预检（issue #1283）、发布（issue #1284）与引导（issue #1285）的锁跨度：
//! manifest GET、封包与上传、整库快照下载在网络在途时不得占据写连接锁——
//! 负向判据（ADR-0087 断言强度）把网络调用移回锁内即红；引导下载在途落进的
//! 本机写被显式拒绝、不静默覆盖（引导侧复验语义的「删除即变红」权威归域
//! 单测，见 sync-engine 域 checkpoint 测试）。
