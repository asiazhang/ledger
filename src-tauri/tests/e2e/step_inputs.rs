...
//! 会把矩阵知识摊回调用点。dividend/split 的合法形态带标的等 per-kind 热点，而
//! 当前调用点均为「缺标的被域守卫拒绝」场景、经通用工厂即可，故不设专用构造函数。
//!
...
///   断言路径）；
/// - dividend（ADR-0109）与 split（ADR-0106）创建守卫场景——如缺标的——经
///   本工厂走通用步骤，断言码化域守卫错误；两者均有 per-kind 热点字段
///   （标的），故通用步骤不构造合法形态。
///
