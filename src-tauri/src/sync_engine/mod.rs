//! 多端同步域（issue #855 / ADR-0091）：OpLog 基座——同步元数据落库、op 产出
//! 信封、跨端全序与幂等重放的完整闭环。
//!
//! 接缝：
//! - [`engine`]（同步引擎公开接口）：[`engine::apply_ops`]（幂等重放）、
//!   [`engine::read_ops`]（日志读取，全序返回）、[`engine::total_order`]（跨端
//!   全序）——本域行为的唯一断言权威层（域单测）。
...
pub use engine::{ApplyReport, OpOutcome, apply_ops, read_ops, total_order};
