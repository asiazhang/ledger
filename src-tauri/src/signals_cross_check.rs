...
/// `pub(crate)`：触发接线守门同用本器具（#959 两个测试随 #1107 同住本文件，
/// 扫描对象含 crates/sync-engine 域内 scheduler.rs）——扫描器具单一维护点，
/// 规则漂移两处同红。
...
// ---------------------------------------------------------------------------
// 触发接线的源码扫描守门（issue #959 / ADR-0098 决策 4；本文件先例的文本级
// 核对，仅测试可见）：触发编排的「接线」行为半边由 mock 应用集成测试
//（`tests/commands/sync_trigger.rs`）钉住，但「分平台门收在 start_triggers
// 单点、三业务可用起点统一调 start_triggers」是源码形状事实——删掉 lib.rs 的
// start_triggers 调用或把门搬回调用点，行为测试无从得知（集成测试不运行
// setup），只有扫描即红。
//
// #1107 起同步域拆独立 crate（ledger-sync-engine）：两个测试的断言与形状
// 零改动、宿主随扫描对象落在根包——① 的扫描对象是壳层接线（lib.rs 与三
// 起点命令），修复落点在壳层（ADR-0087 断言归属）；② 的扫描对象在 crate 内
//（`crates/sync-engine/src/trigger/scheduler.rs`，路径带 crates/ 前缀）。掩码
// 器具仍只消费本文件 [`mask_non_code`] 单一维护点，不在域 crate 内复制第二份。
// 域内对触发入口的合法直呼随域迁出根包 src，天然不在 ① 的扫描范围（原
// `src/sync_engine/` 排除分支随之消亡）。
// ---------------------------------------------------------------------------
