...
//! 同一口径驱动多处：`list_instruments` 的 `invested` 派生列、标的页「只看持仓」
//! 过滤（`only_invested`）、持仓页签概览，以及价格过期提示的「持仓缺现价」判定
//! （issue #1190）。消费处「增量同步的标的收集与统计」自 issue #827 起随同步
//! 覆盖面放开至库内全部标的、按通道能力分区退役，不再消费本谓词。**清单只列
//! 消费面、不写处数**——消费处随功能增减，写死数字必漂移。未来任何「持仓标的」
//! 消费方（如定投提醒）只需复用本常量。
//!
