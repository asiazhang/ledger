...
//! 周点落库原语 `weekly::commit_price_history_weekly` 的嵌套感知自保障，spec #1677：
//! 原语从不在事务外写、裸连接调用即自持一只一事务）。
//!
...
/// 单只标的的落库（生产通道形态：一只标的一个事务——周点落库原语
/// `weekly::commit_price_history_weekly` 的嵌套感知自保障，spec #1677）：现价行 upsert +
