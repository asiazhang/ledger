...
//!   触发已收口连接层统一写入口 [`crate::db::write`]（ADR-0032）：交易写入域、
//!   参考 CRUD 与物品/投资域均已接入；迁移期仅批量导入仍直接调用本函数，
//!   迁入后调用点清零（#245）；深度模块只持有 `&Connection`，
...
/// 交易写入域、参考 CRUD 与物品/投资域均已接入）；迁移期剩余调用点仅批量导入，
/// 随 #245 迁入写入口后清零。本函数随之收归连接层内部（#246）。
///
