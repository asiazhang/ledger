...
//!   触发已收口连接层统一写入口 [`crate::db::write`]（ADR-0032）：交易写入域已接入，
//!   其余写路径（参考 CRUD / 物品 / 投资 / 批量导入）迁移期仍直接调用本函数，
//!   逐域迁移后调用点清零（#243/#244/#245）；深度模块只持有 `&Connection`，
//!   不经 AppHandle 取偏好——目录镜像经进程级单例 [`shared_prefs`] 读取，
//!   应用版本用编译期常量。
//!
...
///
/// 1. 置脏：任何业务写库成功后调用；失败仅记日志不上抛——业务写已成功，
///    不能因调度状态写入失败回滚用户操作。
/// 2. 写时顺带检查：若到期且脏则立即执行自动备份（决策全在 [`due_decision`]，
///    备份频率上限每天一次；目录未配置/开关关闭等门禁由 [`run_due_backup`] 统一处理）。
///
/// **调用方**：连接层统一写入口 [`crate::db::write`]（ADR-0032，提交点单点触发，
/// 交易写入域已接入）；迁移期剩余调用点（参考 CRUD / 物品 / 投资 / 批量导入）
/// 随 #243/#244/#245 迁入写入口后清零。本函数随之收归连接层内部（#246）。
///
/// 处于显式事务中（迁移期批量导入逐行落库仍会命中）时只置脏、不做到期检查——
/// VACUUM INTO 不能在事务内执行；提交点会再调一次本函数补上检查。重复调用安全：
/// 置脏幂等，未到期检查恒为 Skip。
