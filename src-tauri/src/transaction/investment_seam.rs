//! 交易×投资接缝（spec #1086 / issue #1092，挂载点反转形态与 #1090 同族）：投资
//! kind（buy/sell/convert/split/dividend）的写路径计划装配/副作用与读路径投影的
//! 注册点与委派单点。
//!
//! 形态与 [`super::write_effects`]（余额刷新）及 [`super::read`] 计划来源解析同构
//! ——「下层定义注册点、上层注册实现、壳层启动时接线」（ADR-0112 决策 5）：本域
//! 只承诺调用时机与契约类型（[`InvestmentPlan`] 计划契约 + 三份命令字段），投资
//! 语义（标的/持仓/FIFO/批次重述校验与落库）全部留在投资域实现侧
//! （`investment::transaction_seam::install_transaction_hooks`），壳层启动接线。
//! 核心交易域对投资域的直接依赖边随之消除——核心交易 crate（#1092）对任何业务
//! 域零依赖，本模块是「投资域 → 核心交易域」单向边的注册面。
//!
//! 未注册即接线缺失：码化错误上抛（写入/读取显式失败，不静默丢装配或投影）。
...
use super::command::{ConvertCommandFields, InvestmentCommandFields, SplitCommandFields};
use super::model::{ConvertFields, NormalizedTransaction, TransactionInput, TransactionSource};
...
/// 对应 [`super::command::TransactionCommand`] Create/Update 的三个可选语义字段。
///
