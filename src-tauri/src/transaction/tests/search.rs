...
// -----------------------------------------------------------------------
// 进程内候选缓存（issue #493 / ADR-0027 修订记录预留手段）：关键字纯路径走
// 缓存，「写入立即可搜」语义经写后失效保持零变更
// -----------------------------------------------------------------------

/// 走 Writer 接缝插一笔备注交易（直调域函数形态：不经 db::write wrapper，
/// 失效由 Writer 挂点承接——与产品内部路径同款）。
fn insert_row_with_note(conn: &Connection, note: &str, date: &str) -> String {
    let row = NormalizedRow {
        kind: crate::transaction::TransactionKind::Expense,
        amount_cents: 1000,
        currency_code: "CNY".into(),
...
#[test]
fn cache_path_write_then_search_stays_fresh() {
    let conn = setup();
    insert_account(&conn, "a1", "现金", "cash", "CNY");

    // 首搜（缓存路径）：重建快照，命中。
    let id = insert_row_with_note(&conn, "万科物业", "2026-02-01");
    assert_eq!(search(&conn, "wkwy").unwrap().total, 1);

    // 同连接改备注（Writer 直调形态）后再搜：旧词消失、新词命中（写后失效）。
    let changed = NormalizedRow {
        kind: crate::transaction::TransactionKind::Expense,
        amount_cents: 1000,
        currency_code: "CNY".into(),
        amount_native_cents: 1000,
    ...
    update_row(&conn, &id, &changed).unwrap();
    assert_eq!(
        search(&conn, "wkwy").unwrap().total,
        0,
        "改备注后旧词不再命中（写后失效，不得命中陈旧快照）"
    );
    assert_eq!(search(&conn, "zsyh").unwrap().total, 1, "新词即时可搜");

    // 行为层软删后再搜：命中消失（软删 UPDATE 挂点失效）。
    super::super::behavior::delete(&conn, &id).unwrap();
    assert_eq!(search(&conn, "zsyh").unwrap().total, 0, "软删后不再命中");
}

#[test]
fn cache_path_matches_wide_filter_sql_path() {
    // 语义一致性：纯关键字（缓存路径）与 关键字 + 恒宽金额筛选（SQL 组合
    // 路径）必须给出同一命中集合——缓存只是加速，语义零变更。
    let conn = setup();
    insert_account(&conn, "a1", "现金", "cash", "CNY");
    insert_row_with_note(&conn, "万科物业缴费", "2026-02-01");
    insert_row_with_note(&conn, "京东购物 wkwy", "2026-02-02");
    insert_row_with_note(&conn, "无关备注", "2026-02-03");

    let cache = search(&conn, "wkwy").unwrap();
    let sql = search_transactions_internal(
        &conn,
        "wkwy",
        1,
        20,
    ...
    .unwrap();
    assert_eq!(cache.total, sql.total, "两路径 total 一致");
    assert_eq!(
        cache.items.iter().map(|t| t.id.clone()).collect::<Vec<_>>(),
        sql.items.iter().map(|t| t.id.clone()).collect::<Vec<_>>(),
        "两路径命中序一致"
    );
    assert_eq!(cache.total, 2, "拼音子序列与原文子串各命中一笔");
}

#[test]
fn wrapper_committed_write_invalidates_cache() {
    // 产品写路径形态：经 db::write wrapper（提交点 after_commit 单点失效）。
    ...
    state
        ...
        .unwrap();
    let id = state
        .write(|conn| Ok(insert_row_with_note(conn, "万科物业", "2026-02-01")))
        .unwrap();

    // 首搜建快照，再经 wrapper 改备注，再搜：失效生效。
    assert_eq!(
        search(&state.conn.lock().unwrap(), "wkwy").unwrap().total,
        1
    );
    state
        .write(|conn| {
    ...
    assert_eq!(
        search(&conn, "wkwy").unwrap().total,
        0,
        "wrapper 写路径提交后缓存失效（旧词消失）"
    );
    assert_eq!(search(&conn, "ytbh").unwrap().total, 1, "新词即时可搜");
}
