...
- **落账币种必须等于目标账户币种**（写入守卫，issue #1770 / ADR-0134）：每笔交易的 `currency_code` 必须与 `account_id` 所指账户的币种一致，转账另要求 `to_account_id` 账户同币种——先 `GET /api/v1/accounts` 核对目标账户币种再定 `currency_code`。外币刷卡（美元卡刷人民币消费）按**入账账户币种**记录卡组织折算后的金额，后端不折算；换汇（两种货币间的资金移动）**拆成两笔各自同币种的记录**（或走余额调整），不得落一笔与账户异币种的行，否则整行 400 码化拒绝（`transaction.currency-mismatch`）。
- 日期严格 `YYYY-MM-DD`。
...
- **何时需要读**：会话中出现投资类流水（`buy` / `sell` / `convert` / `split` / `dividend`）或投资标的，即须先拉取投资节再落账，且整场会话黏住——对账与纠错口径一并补读；投资类记录不得套用普通收支写法。
...
- 仅 `income` / `expense` / `transfer` 可携带商户；投资流水的商户规则不在本页陈述，见投资节（获取时机见上方知识索引）；退款（refund）自动继承原支出商户，携带的商户会被忽略。
...
- `credit` 是信用卡账户类型（花呗、白条同归）：余额为负表示欠款，还款、刷卡一律记普通 `transfer` / `expense`，不为信用卡另造交易类型。
- 账户可携带三个**档案字段**：`credit_limit_cents`（信用额度，整数分，> 0）、`statement_day` / `due_day`（账单日 / 还款日，1–31 的「每月第 N 日」声明值）。这三个字段**只是档案，不进任何金额与折算口径**：不要用它们算余额、净资产或收支；「这张卡还欠多少」永远看账户余额。仅 `credit` 账户可携此三字段，其他类型携带会被拒。
- 建卡时已知额度 / 账单日就随创建一并写入；未知就留空（AI 不猜、不编造），事后由用户补填。账单日与还款日不强制先后（银行存在跨月形态）。
