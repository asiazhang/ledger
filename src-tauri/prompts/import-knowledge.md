...
- 读回发现商户名写错（乱名、非规范名）→ `PUT /api/v1/merchants/{id}` 改名（新名放 `name`，首尾空白后端修剪）：改名即时生效，历史交易照常引用该商户；改名即时生效后重跑同批导入，新交易按新名精确命中复用同一商户。
- 改名不是合并：改成已有在用名会被拒绝（400 `merchant.already-exists`）。碎商户（同义名分裂）请在导入前先拉商户列表、按已有名字提交预防，不要指望导入后改名归一。

## 投资交易（buy / sell）

- 标的解析三步法（带代码标的的权威路径，与「基金申赎」节对称，不依赖本地标的字典）：① **先按代码查询**：`GET /api/v1/stocks/{code}` 确认识别（命中返回东财权威名称、精确市场、币种、最新价与类型提示 stock/etf；`market` 可选、缺省按代码形态推断——沪深 6 位、港股 5 位及以下左补零、纯字母 ticker 遍历美股三市场（命中返回精确交易所 nasdaq/nyse/amex 与 USD 币种）；北交所代码显式 400 暂不支持）；② **再以真实代码创建**：`POST /api/v1/instruments` 传 `symbol` = 真实代码、`type` 按类型提示填（stock/etf）、`market` = 查询返回的精确市场，后端经东方财富校验并回填权威名称、落最新价现价（导入后持仓立有市值）；查无此码（400 中文报错）请核对代码或跳过该行；东财临时不可达时创建仍成功（降级为提交名称建行、市场保留，行情恢复后价格同步可达），无需重试、不阻塞导入；③ 用返回的标的 `id` 填 `instrument_id` 写 buy/sell。同码异类型（如基金 000001 与股票 000001）经查询端点天然分流。
- 源数据确无代码时才以名称充代码兜底建行（`symbol` = 名称，不触发东财校验、无行情通道）；`GET /api/v1/instruments` 搜索可作名称消歧的辅助，不是带代码行的解析前提（`query` 必填、按 symbol 排序封顶返回，`limit` 缺省 20、上限 100；命中语义：`query` 按空白切词、词条之间 AND，对「代码 · 名称」判定原文连续子串或拼音首字母串，如 `gzmt` 命中 600519 贵州茅台）。
- 标的创建按（代码，类型）幂等：同码同类型静默复用、按需更新名称/市场并返回既有 id（201 + 裸 id 字符串），重跑不产生字典碎片。
...
- 标的解析差异（三步法见「投资交易」节，此处只教差异）：基金账单必带 6 位代码——先 `GET /api/v1/funds/{code}` 按代码查询确认识别（命中返回名称/东财分类/最新净值/净值日期，优先于通用搜索）；再以**真实 6 位代码**作 `symbol`、`type: "fund"` 创建标的，后端会经东方财富校验并回填权威名称、落最新净值现价。fund 标的经查询通道创建后 `market` 恒 unknown、币种恒人民币（显式传参不生效）；基金**不走名称充代码**：查无此码（400 中文报错）请核对代码或跳过该行，仅源数据确无代码时才以名称充代码建行（该行不进净值通道）。东财临时不可达时创建仍成功（后端降级用提交名称建行），无需重试、不阻塞导入。
...
- 单行业务校验失败（金额/转账/商户/标的不存在等）该行返回 `success: false` 附中文 `error`、不影响其余行；`kind` 非法属请求体格式错误，整批 4xx。
...
- 写错的单笔交易 → `PUT /api/v1/transactions/{id}` 按 id 全字段替换（`idempotency_key` 不可编辑，修改后重跑同批导入仍按同键去重、不产生重复；校验规则与创建一致，不存在的 id 返回 404），**不要「删后重导」**。
- 整笔移除（该行本就不该存在）或误建的账户/分类 → 软删除：`DELETE /api/v1/transactions/{id}`、`DELETE /api/v1/accounts/{id}`、`DELETE /api/v1/categories/{id}`（软删不占去重位）。
- 商户名写错 → `PUT /api/v1/merchants/{id}` 改名（见「商户」节；改名即时生效，无需重导）。
