...
- 迁移拆行：通用行只产生 `income` / `expense` / `transfer` 三类 kind；投资流水走 `buy` / `sell`（见「投资交易」）与 `convert`（基金转换，见「基金转换」）。`convert` 可用；`dividend` / `split` 仍不受支持，提交会被拒绝。
...
- 多腿拆单：一笔源转换拆多腿时，按**单腿一条记录**提交多条 `convert` 记录、每腿一条，每条带自己的转出份额与转出金额。各腿转出份额有**逐腿直读** / **金额占比拆分**两写，按快照实际形态择一（这是快照口径假设，迁移侧须在真实数据上核对后固定其一，勿在同批混用）：① **逐腿直读**——每腿快照记录自带转出份额（`share` / `successShare`，如一笔转出 006793 拆两腿读得 684.76 / 2791.12 份），直接填 `quantity`；② **金额占比拆分**——快照只给转出总份额时，按各腿转出金额占比拆分总份额、**尾差末腿**。同一转出标的的金额与份额成比例，两写结果应一致。
...
- 幂等键：与全节同规（`{源文件名}:{行号}`），多腿用 `{源文件名}:{行号}:{腿序}` 派生各腿独立键（如 `:1` / `:2`）——腿序是身份的一部分，勿复用同键。
- 纠错：**修改接口不允许把 kind 改成 / 改出 `convert`**（码化中文错误）——存量「假卖出 + 假买入」按「软删假买 / 假卖（删买级联软删其在用卖出）→ 重建 `convert` → 按快照重导被级联的后续卖出」纠错，全程走契约接口、「不删后重导」纪律不变；转换转出份额不得超当前持仓；误录的转换软删即整笔回退（级联软删其在用后续卖出并精确回补持仓与批次）；但转入批次已被后续在用转换消耗时删除被守卫拒绝（否则会抹掉下游转换的转出消耗记录），须从最后一腿往前处理。
...
- 整笔移除（该行本就不该存在）或误建的账户/分类 → 软删除：`DELETE /api/v1/transactions/{id}`、`DELETE /api/v1/accounts/{id}`、`DELETE /api/v1/categories/{id}`（软删不占去重位）。删除 buy/sell 会同步回补持仓（删买入还级联软删其在用卖出），转换删除同理（见「基金转换」节），重导前无需手工清理持仓残留（issue #940）。
