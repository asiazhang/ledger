...
- 纠错：份额调整的纠错只有**软删 + 重建**一条窄路——进 / 出 `split` 的 kind 变更（把其他类型改成 `split` 或反向）一律拒绝；`split` 行当前**不可改、不可删**（`PUT` / `DELETE` 返回码化 400，过渡期限制），此期间发现误录时不要用买 / 卖借位掩盖、也不要手工改持仓，报告用户等待写入口径更新。
