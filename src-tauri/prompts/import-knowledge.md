...
- 源数据确无代码时才以名称充代码兜底建行（`symbol` = 名称，该行不触发东财校验、不进行情通道）；`GET /api/v1/instruments` 搜索仅作名称消歧的辅助（参数与命中语义见契约端点描述），带代码行的解析走三步法。
...
- 纠错：错误行按 400 中文报错自纠——`instrument_id` 不存在时重走三步法拿正确 id 再提交；`sell` 数量不得超过当前持仓；已有部分卖出的买入禁改禁删（改持仓历史会破坏已实现盈亏）。
