...
- 标的解析三步法：① `GET /api/v1/instruments` 按源数据中的标的描述（代码/名称/拼音首字母）搜索标的；② 未命中再 `POST /api/v1/instruments` 幂等创建（重复创建返回同一 id）：`symbol` 必填，源数据只有名称时以名称充当代码；`type` 从上下文判断（stock/fund/bond/etf/other）；`name` / `market` / `currency_code` 可省；③ 用返回的标的 `id` 填 `instrument_id`。同码异类型靠搜索的 `type` 参数消歧。场外基金（基金申赎）的差异约定见「基金申赎」节。
