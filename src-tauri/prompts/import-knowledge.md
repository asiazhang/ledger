...
- 改名不是合并：改成已有在用名会被拒绝（400 `merchant.already-exists`）；碎商户（同义名分裂）靠导入前预防：先拉商户列表、按已有名字提交。

## 投资交易（buy / sell）

- 标的解析三步法（带代码标的的权威路径，与「基金申赎」节对称，不依赖本地标的字典）：① **先按代码查询**：`GET /api/v1/stocks/{code}` 确认识别（命中返回东财权威名称、精确市场、币种、最新价与类型提示 stock/etf）；`market` 可省——缺省由服务端按代码形态单点推断（推断表见契约端点描述），无需自猜市场；② **再以真实代码创建**：`POST /api/v1/instruments` 传 `symbol` = 真实代码、`type` 按类型提示填（stock/etf）、`market` = 查询返回的精确市场，后端经东方财富校验并回填权威名称、落最新价现价（导入后持仓立有市值）；查无此码（400 中文报错）请核对代码或跳过该行；东财临时不可达时创建仍成功（降级为提交名称建行、市场保留，行情恢复后价格同步可达），无需重试、不阻塞导入；③ 用返回的标的 `id` 填 `instrument_id` 写 buy/sell。同码异类型（如基金 000001 与股票 000001）经查询端点天然分流。
- 源数据确无代码时才以名称充代码兜底建行（`symbol` = 名称，不触发东财校验、无行情通道）；带代码行的解析走三步法，`GET /api/v1/instruments` 搜索仅作名称消歧的辅助（参数与命中语义见契约端点描述）。
...
- 行拆解：申购 → `buy`、赎回 → `sell`，行字段同「投资交易」节；`quantity` = 确认份额（份）、`amount_cents` = 确认单整分金额（**权威事实**，原样提交：申购为含费扣款额、赎回为减费到账额）、`fee_cents` = 申购/赎回费（分，归属费用字段）。**不传 `price_cents`**：成交净值由服务端按（金额 ∓ 费用）÷ 份额反算（万分之一元，4 位小数保真），行金额恒等于确认单金额、不被净值舍入污染（金额口径与「投资交易」节的重算公式相反）。
...
- 读回过滤参数与语义全表见契约 transactions 端点（全部可选，示例见上方查询纪律）；默认按日期倒序稳定排序，翻页无重复无遗漏。
