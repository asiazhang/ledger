...
| 3 | 投资域 | [`docs/contexts/CONTEXT-investment.md`](docs/contexts/CONTEXT-investment.md) | Instrument、MarketPrice、PriceHistory、价格通道（PriceChannel）、FxRateHistory、PortfolioValueTrend、Holding、NetWorth、InvestableAssets、FinancialFreedom、时点持仓、Investment、Unwind（持仓副作用撤销）、基金转换（Conversion）、TransactionTrade、TransactionConvert、InvestedInstrument、已实现盈亏（RealizedPnl）、累计收益（CumulativePnl）、自建标的、手动报价、行情接入（QuoteAdoption）、价格刻度、份额容差（QuantityTolerance）、InstrumentSync（已退役→按代码查询/创建）、InstrumentInfoSync（前名 HoldingPriceSync）、价格失效信号 |
