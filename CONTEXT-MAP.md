...
| 3 | 投资域（三卷：交易与持仓 / 资产与收益 / 标的与价格） | [`docs/contexts/CONTEXT-investment.md`](docs/contexts/CONTEXT-investment.md)、[`docs/contexts/CONTEXT-investment-aggregates.md`](docs/contexts/CONTEXT-investment-aggregates.md)、[`docs/contexts/CONTEXT-investment-prices.md`](docs/contexts/CONTEXT-investment-prices.md) | Instrument、市场（Market）、MarketPrice、PriceHistory、价格历史后台补全（PriceHistoryBackfill）、价格通道（PriceChannel）、价格恒定标的（Constant-Price Instrument）、行情批量取数（BulkQuoteFetch）、取数单元、源畸形（Source Malformed）、取数尾部契约（Fetch Tail Contract）、通道束（SyncFetchChannels）、周点落库（Weekly Point Commit）、FxRateHistory、PortfolioValueTrend、Holding、NetWorth、InvestableAssets、FinancialFreedom、跨账本投资汇总（CrossBookInvestmentSummary）、投资概览（InvestmentOverview）、时点持仓、Investment、Unwind（持仓副作用撤销）、共享守卫（SharedGuard）、基金转换（Conversion）、份额调整（Split）、现金分红（Dividend）、红利再投（DividendReinvestment）、投资明细页签（Investment Ledger Tab）、TransactionTrade、TransactionConvert、InvestedInstrument、盈亏展示口径、已实现盈亏（RealizedPnl）、已实现收益（RealizedGain）、累计收益（CumulativePnl）、资金加权收益率（MoneyWeightedReturn）、期初存量（OpeningBalance）、自建标的、手动报价、行情接入（QuoteAdoption）、价格刻度、份额容差（QuantityTolerance）、InstrumentSync（已退役→按代码查询/创建）、InstrumentInfoSync（前名 HoldingPriceSync）、价格过期提示（PriceStalenessPrompt）、价格失效信号 | |
...
| 12 | 测试基础设施 | [`docs/contexts/CONTEXT-testing.md`](docs/contexts/CONTEXT-testing.md) | invoke 测试接缝、defaults 表、overrides 表、未命中报错、参考数据预热、清理四件套、消息替身稳定实例、目录级测试薄壳、行为等价判据（ADR-0085）；测试世界、步骤输入工厂、步骤动词、快照分组、公开写入口（测试侧）（ADR-0086）；测试三层与权威层、壳三件套、接线证明、错误码契约代表（ADR-0087）；断言强度、双断言、矩阵、存在性断言、组件测试数据工厂；零击杀测试、存活变异体、等价变异体（ADR-0137） |
...
- **界面状态与交互 → 只读消费各域**：WindowState / ViewState / 快捷键 / 弹层抑制 / 弹层关闭语义 / 组内收纳（ADR-0063）是纯界面层概念，不持业务数据；各模块的跨域边一律只读——TransactionFilter 消费参考数据就绪状态与 URL 下钻参数（筛选不持久化，ADR-0030）、ScheduledPlanList 经定时计划域命令变更生命周期（ADR-0041）、来源列反查既有指针不新增数据级反向引用、拼音可搜下拉与交易搜索同源核心域 TransactionSearch 词条的统一模糊搜索语义（ADR-0027），拼音首字母子序列分支自语义契约定稿（ADR-0027 修订 / #1728）归下拉独享、交易搜索只按原文命中、功能开关（见参考设置域）只收窄入口不改写收纳清单（ADR-0116）。
