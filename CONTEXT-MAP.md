...
| 3 | 投资域 | [`docs/contexts/CONTEXT-investment.md`](docs/contexts/CONTEXT-investment.md) | Instrument、MarketPrice、PriceHistory、价格通道（PriceChannel）、FxRateHistory、PortfolioValueTrend、Holding、NetWorth、InvestableAssets、FinancialFreedom、跨账本投资汇总（CrossBookInvestmentSummary）、时点持仓、Investment、Unwind（持仓副作用撤销）、基金转换（Conversion）、份额调整（Split）、现金分红（Dividend）、红利再投（DividendReinvestment）、TransactionTrade、TransactionConvert、InvestedInstrument、已实现盈亏（RealizedPnl）、累计收益（CumulativePnl）、资金加权收益率（MoneyWeightedReturn）、自建标的、手动报价、行情接入（QuoteAdoption）、价格刻度、份额容差（QuantityTolerance）、InstrumentSync（已退役→按代码查询/创建）、InstrumentInfoSync（前名 HoldingPriceSync）、价格过期提示（PriceStalenessPrompt）、价格失效信号 |
...
| 5 | 参考数据与设置 | [`docs/contexts/CONTEXT-reference-settings.md`](docs/contexts/CONTEXT-reference-settings.md) | Reference Data、信用卡档案（Credit Terms）、BalanceAdjustment、Appearance、界面语言、应用名称、AppSettings、轻量设置项、功能开关、LedgerLevelSetting、日志等级、金额隐私模式、账本（Book）、DataLocation |
...
- **核心交易是被所有域消费的底座**。Transaction 与 Amount Model（raw/native 分离 + kind→度量符号矩阵）定义「一笔资金变动长什么样」，Category / Merchant / DefaultCurrency 是它依赖的字典与折算基准；定时计划、投资、AI 导入、物品四域产出的最终都是核心域的交易流水（或以它为对账基准），所以这些共享概念只在核心域定义；跨域共享的失效信号家族术语同样定义于此，成员按域限定（参考失效信号 / 价格失效信号 / 备份信号），各域不复制家族定义。
...
- **投资域 → 核心交易**：buy/sell 首先是核心域 Transaction 的 kind（场外基金申赎同构复用，ADR-0038），资金流出/流入经核心域出资账户归因（ADR-0096），Investment 是它背后的持仓/盈亏载体；现金分红（Dividend）同为投资域消费的核心域 kind——归属某标的、按现金腿入到账账户并计入累计收益第三腿（ADR-0109）；红利再投（DividendReinvestment）不新增 kind，由「现金分红到账投资账户 + 0 手续费买入自投资账户出资」两笔组合表达（两腿在投资账户内自相抵、外部资金不动，ADR-0109 2026-09-13 修订）；市值与净资产折算消费核心域 DefaultCurrency，历史折算经本域 FxRateHistory。财务自由度（FinancialFreedom）以本域可投资资产（InvestableAssets）为分子、跨域消费预算域年度预算总额（AnnualBudgetTotal）为分母（ADR-0048）；跨账本投资汇总（CrossBookInvestmentSummary）是账本「不跨本汇总」边界下投资域的唯一跨本例外（ADR-0114）。
- **AI 导入 → 核心交易 + 参考数据 + 投资域**：AI 经本域 AI API 幂等写入核心域 Transaction 与参考数据（账户/分类/币种/商户）及投资域标的（buy/sell 迁移先经按代码查询 → 真实代码幂等创建解析为标的 id，本地字典搜索降为辅助，ADR-0081；场外基金按真实代码建行，ADR-0039），ImportDedup / IdempotencyKey 是写入侧的去重契约，读回验证（AIReadbackVerification）按核心域 InvolvingAccount 与余额口径对账；BlackHoleAccount 是参考数据中的特殊账户，因由导入流程预置与消费而归本域。会话形态分 AI 记账（日常账单增量，多数与投资无关）与数据迁移两类；导入知识按域分级——知识索引 + 分域知识节，投资五节走独立端点按需获取，记账会话不再整份拉取（ADR-0110）。
- **参考数据与设置 → 被核心交易引用、与备份相邻**：Reference Data（账户/分类/币种字典）被核心域 Transaction 以外键引用；AppSettings 是后端配置与运行时状态的权威落点，备份域的调度状态（AutoBackup / DirtyMarker）存于其中——这是它与备份域的相邻点；账本（Book）与账本注册表也归本域（术语归属）：多本账的清单与活动指针是「建连前必须可读」的引导配置，故落库外引导文件而非库内（指针文件泛化，ADR-0089 显式修订 ADR-0018；实现分层见 ADR-0111）；DataLocation 随之收窄为默认数据目录与引导文件所在处，更改位置仅搬迁当前活动账本。信用卡账户的额度与账单日（信用卡档案）也归本域，且**不进任何金额口径**：额度不参与余额、净资产、折算与报表，只作档案与展示（spec #1327 / ADR-0119）。
- **备份与数据文件 ↔ 各域相邻而不交叉**：Backup/Restore 是文件级整库快照通道，与 AI 导入（语义级写入）、行情同步（投资域）互不交叉；备份与自动解锁按账本分域——产物命名携带账本标识、受管清理按本分别滚动、解锁缓存按本隔离（多本语义见参考数据与设置域账本（Book））；备份不迁移界面状态（界面域）与设备偏好（参考设置域；核心域 DefaultCurrency 的前端展示偏好——其本位币基准已升格账本级设置，见参考设置域 LedgerLevelSetting）。
- **界面状态与交互 → 只读消费各域**：WindowState / ViewState / 快捷键 / 弹层抑制 / 弹层关闭语义是纯界面层概念，不持业务数据；组内收纳（每组「更多」聚合页与用户移入移出，ADR-0063，取代 ADR-0055 全局单一收容器）同为纯界面层信息架构——被收视图整体迁入为所在组的页签，只动导航位置、不改域语义与功能；功能开关（见参考设置域）与之两层正交——开关管功能**在不在应用里**、收纳管它**在组内什么位置**，关闭只藏入口且不改写收纳清单（ADR-0116）；TransactionFilter（交易列表过滤模块）消费参考数据就绪状态（参考设置域 Reference Data）与 URL 下钻参数，筛选不持久化（与 ViewState 边界一致，ADR-0030）；ScheduledPlanList（计划清单）把定时计划域 ScheduledTransaction 三形态的清单编排收为深模块、与计划表单接缝构成计划域前端双接缝，生命周期变更仍走定时计划域命令（ADR-0041）；来源列把交易的发起来源实体（计划三形态/保单/物品/标的，均反查既有指针、不新增数据级反向引用）显示为可点击链接，经实体定位参数精确落地，落点尊重组内收纳；搜索（核心域 TransactionSearch）复用交易列表信息但搜索词不持久化（与 ViewState 边界一致）；实体下拉的拼音过滤（拼音可搜下拉）与交易搜索共用统一模糊搜索语义规格（唯一定义点见核心域 TransactionSearch 词条，ADR-0027）。
...
- **测试基础设施 → BDD 步骤层消费各域公开写入口**：步骤动词经域层/编排入口公开函数造数与动作，业务不变量由产品代码保证，裸 SQL 限于无公开入口的库内状态直置并显式登记（ADR-0086）；被测实体与命令的语义归各业务分域，本域只定义测试层的构造、写入与分组纪律。
- **多端同步 → 经写入接缝重放各域语义命令**：同步域不定义业务语义，op 载荷就是各域既有写入命令，重放是行为编排之外的第 N 写入入口（接缝契约与批量导入同待遇，ADR-0091）；与备份域相邻——SyncEnvelope 复用加密模式 / 主口令机制，但备份 ≠ 同步（快照还原 vs 增量合并）；定时计划期次触发经 OccurrenceKey 确定性去重防双扣；同步边界与 Backup 迁移边界对齐，唯一升格成员是 DefaultCurrency 本位币基准（见参考设置域 LedgerLevelSetting）。
- **实物资产（单列小域）→ 核心交易（只消费不产流水）**：PhysicalAsset 是大件实物的估值档案，与物品域 Item 按「要不要跟踪市值」互斥分家（物品摊成本、资产记市值）；估值全手动且每次追加估值历史行（当前值 = 最新一条），金额一律整数分、折本位币走核心域 Amount 接缝（当期汇率，缺汇率错误上抛）；币种复用核心域币种字典；在持估值计入净资产第三腿（ADR-0064），已处置/软删不计、可投资资产（财务自由度分子）不受影响；入口收纳在资产组「更多」页签（ADR-0063）；无交易流水产出，纯 IPC 面，AI 导入不识别（MVP 边界）。
