...
| 3 | 投资域 | [`docs/contexts/CONTEXT-investment.md`](docs/contexts/CONTEXT-investment.md) | Instrument、MarketPrice、PriceHistory、价格通道（PriceChannel）、FxRateHistory、PortfolioValueTrend、Holding、NetWorth、InvestableAssets、FinancialFreedom、时点持仓、Investment、Unwind（持仓副作用撤销）、基金转换（Conversion）、份额调整（Split）、现金分红（Dividend）、TransactionTrade、TransactionConvert、InvestedInstrument、已实现盈亏（RealizedPnl）、累计收益（CumulativePnl）、自建标的、手动报价、行情接入（QuoteAdoption）、价格刻度、份额容差（QuantityTolerance）、InstrumentSync（已退役→按代码查询/创建）、InstrumentInfoSync（前名 HoldingPriceSync）、价格失效信号 |
...
- **投资域 → 核心交易**：buy/sell 首先是核心域 Transaction 的 kind（场外基金申赎同构复用，ADR-0038），资金流出/流入经核心域出资账户归因（ADR-0096），Investment 是它背后的持仓/盈亏载体；现金分红（Dividend）同为投资域消费的核心域 kind——归属某标的、按现金腿入到账账户并计入累计收益第三腿（ADR-0109）；市值与净资产折算消费核心域 DefaultCurrency，历史折算经本域 FxRateHistory。财务自由度（FinancialFreedom）以本域可投资资产（InvestableAssets）为分子、跨域消费预算域年度预算总额（AnnualBudgetTotal）为分母（ADR-0048）。
