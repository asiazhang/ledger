...
| 3 | 投资域 | [`docs/contexts/CONTEXT-investment.md`](docs/contexts/CONTEXT-investment.md) | Instrument、MarketPrice、PriceHistory、价格通道（PriceChannel）、FxRateHistory、PortfolioValueTrend、Holding、NetWorth、InvestableAssets、FinancialFreedom、跨账本投资汇总（CrossBookInvestmentSummary）、时点持仓、Investment、Unwind（持仓副作用撤销）、基金转换（Conversion）、份额调整（Split）、现金分红（Dividend）、红利再投（DividendReinvestment）、TransactionTrade、TransactionConvert、InvestedInstrument、已实现盈亏（RealizedPnl）、累计收益（CumulativePnl）、资金加权收益率（MoneyWeightedReturn）、期初存量（OpeningBalance）、自建标的、手动报价、行情接入（QuoteAdoption）、价格刻度、份额容差（QuantityTolerance）、InstrumentSync（已退役→按代码查询/创建）、InstrumentInfoSync（前名 HoldingPriceSync）、价格过期提示（PriceStalenessPrompt）、价格失效信号 |
