...
| 7 | 界面状态与交互 | [`docs/contexts/CONTEXT-ui-interaction.md`](docs/contexts/CONTEXT-ui-interaction.md) | TransactionFilter、数据期间边界、时间范围快捷选择、TransactionModalState（交易弹窗编排）、Loadable（异步任务）、GlobalBusyBar（全局忙碌条）、ScheduledPlanList（计划清单）、WindowState、ViewState、会话内保留、ViewShortcut、侧栏分组、组内收纳、CreateShortcut、Overlay Suppression、弹层关闭语义、ESC 键语义、原生右键菜单、界面文本不可选、拼音可搜下拉、报表年份筛选（已退役→时间范围快捷选择）、分类下钻 |
