...
- 账户管理：账户列表与实时余额计算，支持现金/银行/信用卡/电子钱包/投资/负债/应收等多种账户类型
- 分类管理：收入/支出分类
- 报表分析：按分类、时间维度统计
- 预算：预算设置与跟踪
- 投资管理：投资持仓与市值跟踪
...
- Tauri IPC 是前后端唯一通信方式：后端命令定义在 `src-tauri/src/commands/` 目录下各模块中，并在 `src-tauri/src/lib.rs` 注册；前端统一通过 `src/api/index.ts` 调用 `invoke`。
