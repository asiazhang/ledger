...
export function merchantTableRows(rows: MerchantShare[], totalCents: number): MerchantTableRow[] {
  ...
  return rows.map((r, i) => ({
    ...
    barPct:
      maxPositive > 0 ? Math.round((Math.max(r.amount_cents, 0) / maxPositive) * 10000) / 100 : 0,
