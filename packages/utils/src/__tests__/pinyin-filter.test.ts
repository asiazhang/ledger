...
import { isSubsequence, matchLabel, pinyinFilter, pinyinInitials } from "../pinyin-filter";
...
describe("pinyinFilter（NSelect filter 签名收口）", () => {
  const option = (label: string): SelectOption => ({ label, value: label }) as SelectOption;
