...
describe("期间面板时间戳转换", () => {
  it("按本地日历反推月/季/年，并取自然期间起点", () => {
    ...
    expect(periodFromTimestamp("month", timestamp)).toEqual({
      unit: "month",
      year: 2026,
      index: 4,
    });
    expect(periodFromTimestamp("quarter", timestamp)).toEqual({
      unit: "quarter",
      year: 2026,
      index: 1,
    });
...
describe("presetRange：预设 → 含边界日期区间（本地自然周期）", () => {
  ...
  it("当季：季初月份 1/4/7/10 分别落到四个自然季度（1–3、4–6、7–9、10–12）", () => {
    ...
    expect(presetRange("quarter", d(2026, 4, 15))).toEqual({
      from: "2026-04-01",
      to: "2026-06-30",
    });
    expect(presetRange("quarter", d(2026, 7, 31))).toEqual({
      from: "2026-07-01",
      to: "2026-09-30",
    });
    expect(presetRange("quarter", d(2026, 10, 1))).toEqual({
      from: "2026-10-01",
      to: "2026-12-31",
    });
  });
  ...
  it("当季：闰年 2 月 29 日仍属一季度，季度边界不受闰月影响", () => {
    expect(presetRange("quarter", d(2024, 2, 29))).toEqual({
      from: "2024-01-01",
      to: "2024-03-31",
    });
  });
  ...
  it("去年：当前年减一的完整自然年（年初跨年、年末同区间）", () => {
    expect(presetRange("lastYear", d(2026, 1, 1))).toEqual({
      from: "2025-01-01",
      to: "2025-12-31",
    });
    expect(presetRange("lastYear", d(2026, 12, 31))).toEqual({
      from: "2025-01-01",
      to: "2025-12-31",
    });
  });
...
describe("rangeToPeriod ⇄ periodRange：区间 ⇄（单位，期间）双向换算（issue #383）", () => {
  ...
  it("具体区间反推：月/季/年与跨月历史周期各命中正确单位", () => {
    expect(rangeToPeriod("2026-02-01", "2026-02-28")).toEqual({
      unit: "month",
      year: 2026,
      index: 1,
    });
    expect(rangeToPeriod("2024-02-01", "2024-02-29")).toEqual({
      unit: "month",
      year: 2024,
      index: 1,
    });
    expect(rangeToPeriod("2026-01-01", "2026-03-31")).toEqual({
      unit: "quarter",
      year: 2026,
      index: 0,
    });
    expect(rangeToPeriod("2025-10-01", "2025-12-31")).toEqual({
      unit: "quarter",
      year: 2025,
      index: 3,
    });
    expect(rangeToPeriod("2026-01-01", "2026-12-31")).toEqual({
      unit: "year",
      year: 2026,
      index: 0,
    });
    expect(rangeToPeriod("2025-01-01", "2025-12-31")).toEqual({
      unit: "year",
      year: 2025,
      index: 0,
    });
  ...
  it("跨多年全枚举：所有月份与季度单调有序且边界包含正确", () => {
    for (let startY = 2023; startY <= 2026; startY++) {
      for (let endY = startY; endY <= 2027; endY++) {
        ...
        expect(bounds.month.latest.index).toBe(
          Math.max(2026, endY) === 2026 && endY < 2026 ? 2 : 11,
        );

        // 季档
        expect(comparePeriods(bounds.quarter.earliest, bounds.quarter.latest)).toBeLessThanOrEqual(
          0,
        );
...
describe("comparePeriods & isPeriodWithinBoundary & clampPeriod：期间比较与边界判定（issue #390）", () => {
  it("comparePeriods：同一单位内跨年、年内索引比较严格一致", () => {
    ...
    expect(
      comparePeriods(
        ...
        { unit: "month", year: 2026, index: 0 },
      ),
    ).toBeLessThan(0);
    expect(
      comparePeriods(
        { unit: "month", year: 2026, index: 5 },
        { unit: "month", year: 2026, index: 3 },
      ),
    ).toBeGreaterThan(0);
    ...
    expect(
      comparePeriods(
        ...
        { unit: "quarter", year: 2026, index: 0 },
      ),
    ).toBeLessThan(0);
    expect(
      comparePeriods(
        { unit: "quarter", year: 2026, index: 2 },
        { unit: "quarter", year: 2026, index: 2 },
      ),
    ).toBe(0);
    ...
    expect(
      comparePeriods(
        { unit: "year", year: 2024, index: 0 },
        { unit: "year", year: 2026, index: 0 },
      ),
    ).toBeLessThan(0);
    expect(
      comparePeriods(
  ...
  it("clampPeriod：低于下界钳制为下界，高于上界钳制为上界，界内原样返回", () => {
    ...
    // 低于下界
    expect(clampPeriod({ unit: "quarter", year: 2023, index: 3 }, boundary)).toEqual(
      boundary.earliest,
    );
    expect(clampPeriod({ unit: "quarter", year: 2024, index: 1 }, boundary)).toEqual(
      boundary.earliest,
    );

    // 高于上界
    expect(clampPeriod({ unit: "quarter", year: 2026, index: 2 }, boundary)).toEqual(
      boundary.latest,
    );
    expect(clampPeriod({ unit: "quarter", year: 2027, index: 0 }, boundary)).toEqual(
      boundary.latest,
    );
  });
});
