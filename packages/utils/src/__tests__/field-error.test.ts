...
describe("judgeQuantityText（数量格式判定闭集，输入粒度至多四位小数）", () => {
  ...
  describe("解析失败（parse-error）", () => {
    it.each(["4.30发", "abc", "1e3", "1,000", "4.3.0", ".", "-", "1 2"])(
      "%s → parse-error",
      (text) => {
...
describe("fieldErrorKind（错误态装配：格式类即时红，空值红在失焦或保存尝试后）", () => {
  ...
  it("空值：失焦后 → 红", () => {
    expect(fieldErrorKind({ kind: "empty" }, { touched: true, saveAttempted: false })).toBe(
      "empty",
    );
  });

  it("空值：保存尝试后 → 红（提交意图触发兜底红态）", () => {
    expect(fieldErrorKind({ kind: "empty" }, { touched: false, saveAttempted: true })).toBe(
      "empty",
    );
  });
