...
describe("categoryBars 一级归并（issue #378）", () => {
  ...
  it("净额 0 的分类不进图", () => {
    const bars = categoryBars(
      [share("food", 0, "餐饮"), share("transport", 3000, "交通")],
      categories,
    );
...
describe("categoryBars 排序与配色（issue #378）", () => {
  ...
  it("同分类 id 颜色稳定：跨调用、跨数据顺序一致", () => {
    const a = categoryBars(
      [share("food", 100, "餐饮"), share("transport", 200, "交通")],
      categories,
    );
    const b = categoryBars(
      [share("transport", 200, "交通"), share("food", 100, "餐饮")],
      categories,
    );
    expect(a.find((x) => x.id === "food")?.color).toBe(b.find((x) => x.id === "food")?.color);
    expect(a.find((x) => x.id === "transport")?.color).toBe(
      b.find((x) => x.id === "transport")?.color,
    );
  });
