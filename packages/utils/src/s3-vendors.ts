 ...
 * 来源是 #1222 产出的「已实测厂商清单」（`docs/verification/1222-s3-vendor-acceptance.md`，
 * 该文档给出跑法、逐家限制与结论回写步骤）。**本表 `verified` 字段是唯一的翻转点**：
 * 真桶实测通过后逐家改写该字段，界面标注随之改变，不在别处复制一份档位表；清单
 * 文档与档位由前端守门测试逐行对齐，单翻一侧即红。
 */
