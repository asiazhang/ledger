 ...
 *
 * 档位标注（`verified`）默认全部 false——目前没有任何厂商跑过真实桶，逐家结论的
 * **唯一翻转点就是本表 `verified` 字段**，不在别处复制一份档位表。真桶验收的跑法、
 * 前置与结论回写步骤见 `src-tauri/tests/real_bucket_acceptance.rs` 的模块文档：按它
 * 跑通某一家后，把该家的 `verified` 改成 true，界面标注随之改变。
 */
