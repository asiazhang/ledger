...
describe("参考数据兜底（REFERENCE_DEFAULTS 经唯一接缝，issue #725/#750）", () => {
  ...
  it("未覆写的非参考命令保持 unexpected invoke 拒绝", async () => {
    wireInvokeSeam();
    await expect(mockInvoke("list_transactions")).rejects.toThrow(
      "unexpected invoke: list_transactions",
    );
...
describe("参考命令覆写（overrides 表优先于规范夹具）", () => {
  ...
  it("覆写参考命令：函数型覆写在派发时以 args 调用", async () => {
    ...
    await expect(mockInvoke("list_accounts", { id: "acc-1" })).resolves.toEqual({
      echoed: { id: "acc-1" },
    });
  });
  ...
  it("函数型覆写可返回 Promise（在途/拒绝场景原样透传）", async () => {
    ...
    let settled = false;
    void mockInvoke("list_categories").then(() => {
      settled = true;
    });
