 ...
 * （顺序 = 菜单顺序 = 组内键位），Pinia store 形态——与同类持久界面状态同归宿。
...
// ---------------------------------------------------------------------------
// 移动纯函数：只见数据、不见 store——写路径把「找组、在册守卫、双存储同步」内化在
// store 侧，纯函数保持可独立直测（移动语义、边界 no-op、不改输入）。
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 菜单构建（文件级纯函数）：order 入参、不吃 store 实例、不持弹层状态、
// 不接弹层注册表（ADR-0041 弹层纯度同款精神）；置灰判定全在入参组内序上，
// 响应式随动由调用方以 computed 包一层（见 App.vue / GroupMoreView.vue）。
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// store 本体：模块级单例 → pinia store（issue #524/#549）。setup 风格（同仓六 store
// 惯例）；启动读路径在 store 首次实例化时执行一次（等价旧模块加载读一次 localStorage），
// 四条写路径收口于此：点选即写、双存储同步、边界 no-op 不写。
// ---------------------------------------------------------------------------

/**
 * 侧栏排序 store：组内序 + 每组有序收纳清单的单一归宿（ViewState 词条覆盖的持久界面状态）。
...
export const useSidebarOrderStore = defineStore("sidebar-order", () => {
   ...
   * applyMoveIntoMore（#474 移入）/ applyMoveBackToSidebar（#475 移回）/ resetSidebarOrder
   * （复位）：四者同步更新此 ref 与持久化，菜单顺序与键位经 viewShortcuts 自动随动。
   */
   ...
   *  组满拒写（≤3 运行时硬上限兜底，菜单置灰为第一道防线）；
   *  非清单成员（在册主项/固定项/未知名）no-op 不写存储。
   *  移回自由度受 ≤3 硬上限约束：组满 3 主项时菜单置灰 + 提示（上限可见、可学习），
   *  不自动换出、不弹窗腾位；写路径同判定兜底拒写。移回组内最后一个收纳成员后
