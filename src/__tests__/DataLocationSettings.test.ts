...
describe("DataLocationSettings.vue", () => {
  ...
  it("目标已有同名库：先弹 warning 级二选一弹窗（按钮语义显式），接管后以 adoptExisting=true 二次提交", async () => {
    const choice: DataLocationChangeOutcome = {
  ...
  it("目录选择被取消时不提交任何更改", async () => {
    ...
    await findButton(wrapper, "更改")!.trigger("click");
    await flushPromises();
    expect(mockInvoke).not.toHaveBeenCalledWith("submit_data_location_change", expect.anything());
  });
  ...
  it("未配置自定义位置时「恢复默认」不生效；已配置时走同一路径可用", async () => {
    ...
    await flushPromises();
    expect(mockInvoke).not.toHaveBeenCalledWith("restore_default_data_location", expect.anything());

    // 已配置自定义位置后按钮可用，走与更改相同的提交流。
    mockOpen.mockResolvedValue("/Volumes/Sync/ledger-data");
    wireInvokeSeam({
      defaults: {
        get_data_location_info: { ...baseInfo, configured_dir: "/Volumes/Sync/ledger-data" },
      },
    });
    await findButton(wrapper, "更改")!.trigger("click");
    await flushPromises();
