...
describe("SyncProgressBar 同步进度条（issue #897）", () => {
  ...
  it("渲染计数文案与进度宽度（done/total → 百分比）", () => {
    ...
    expect(bar.find('[data-testid="instrument-sync-progress-bar"]').attributes("style")).toContain(
      "width: 37%",
    );
  });
  ...
  it("done 为 0 时渲染 0% 起点而非隐藏", () => {
    ...
    expect(
      wrapper.find('[data-testid="instrument-sync-progress-bar"]').attributes("style"),
    ).toContain("width: 0%");
  });
  it("页级明细已随换源退役：仅渲染标的级计数（issue #1571）", () => {
    const wrapper = mountBar({ done: 5, total: 100 });
    expect(wrapper.text()).toContain("同步标的信息 5/100");
