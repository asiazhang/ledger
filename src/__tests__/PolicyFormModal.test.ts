...
async function enableAgreement(wrapper: ReturnType<typeof mount>) {
  ...
  componentVm(wrapper.findComponent('[data-testid="policy-agreement-account"]')).$emit(
    "update:value",
    "acc-1",
  );
  await flushPromises();
...
describe("PolicyFormModal 缴费协议区（issue #362 / ADR-0051 决策 2；不挂商户 #713 / ADR-0082）", () => {
  ...
  it("跳过协议区保存 = 趸交/缴清纯档案：只调 create_policy，不调 create_scheduled_transaction", async () => {
    ...
    componentVm(wrapper.findComponent('[data-testid="policy-insurer"]')).$emit(
      "update:value",
      "ins-1",
    );
  ...
  it("开启协议区保存：先建档再创建订阅形态协议，携带保单引用且不挂商户，备注带险种", async () => {
    ...
    const planCall = mockInvoke.mock.calls.find(([cmd]) => cmd === "create_scheduled_transaction");
