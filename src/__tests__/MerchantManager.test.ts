...
describe("MerchantManager.vue（issue #189）", () => {
  ...
  it("添加商户：调用 create_merchant，重拉后列表出现新商户", async () => {
    ...
    const nameInput = wrapper
      .findAll("input")
      .find((i) => i.attributes("placeholder") === "商户名称")!;
...
describe("MerchantManager.vue 拼音模糊搜索（issue #447，统一模糊搜索语义 ADR-0027）", () => {
  ...
  const searchMerchants: Merchant[] = ["京东", "盒马", "万科物业", "红旗连锁", "盒马鲜生"].map(
    (name, i) => ({
      id: `mch-s-${i}`,
      name,
      updated_at: "2026-01-01T00:00:00Z",
...
describe("MerchantManager.vue 显示已删切换（issue #447）", () => {
  ...
  it("已删行只读：无编辑/删除操作（在用行操作不受影响）", async () => {
    ...
    const deletedRowEl = wrapper.findAll("tbody tr").find((r) => r.text().includes("永辉超市"))!;
    expect(deletedRowEl.findAll("button").filter((b) => b.text() === "编辑")).toHaveLength(0);
    expect(deletedRowEl.findAll("button").filter((b) => b.text() === "删除")).toHaveLength(0);
  });
  ...
  it("已删行条数照常显示且可下钻（软删商户条数后端照常计数）", async () => {
    ...
    const wrapper = mount(MerchantManager);
    await showDeletedToggle(wrapper).trigger("click");
    const deletedRow = wrapper.findAll("tbody tr").find((r) => r.text().includes("永辉超市"))!;
...
describe("MerchantManager.vue 前端分页（issue #457）", () => {
  ...
  async function gotoPage(wrapper: ReturnType<typeof mount>, n: number) {
    const item = wrapper.findAll(".n-pagination-item").find((el) => el.text() === String(n));
  ...
  function mockDeleteMerchant(id: string) {
    wireInvokeSeam({
      overrides: {
        delete_merchant: () => {
          merchantDb = merchantDb.map((m) => (m.id === id ? { ...m, is_deleted: true } : m));
