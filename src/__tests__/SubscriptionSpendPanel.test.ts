...
function makeRow(
  partial: Partial<SubscriptionSpendRow> & { plan_id: string },
...
describe("SubscriptionSpendPanel 订阅花费双口径（issue #160/#161）", () => {
  ...
  it("渲染折算月/年推算成本（只统计进行中订阅，纯展示口径）", async () => {
    const wrapper = mount(SubscriptionSpendPanel);
    await flushPromises();
    expect(wrapper.get('[data-testid="spend-projected-month"]').text()).toBe(
      formatAmount(4030, cny),
    );
    expect(wrapper.get('[data-testid="spend-projected-year"]').text()).toBe(
      formatAmount(48360, cny),
    );
  ...
  it("趋势图渲染过去 12 个月逐月序列（含无扣款月补 0）", async () => {
    ...
    expect(payload.datasets[0].data).toEqual([0, 0, 0, 0, 0, 0, 0, 0, 0, 34800, 0, 3000]);
  });
