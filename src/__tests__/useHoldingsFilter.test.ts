...
describe("sortHoldings 排序语义", () => {
  ...
  it("市值降序：数值递减，缺价行仍恒排末尾（与方向无关）", () => {
    expect(
      ids(sortHoldings(FIXTURE_ROWS, { columnKey: "market_value", order: "descend" })),
    ).toEqual(["h3", "h4", "h1", "h2"]);
  });

  it("未实现盈亏降序：盈利在前亏损在后，缺价行恒排末尾", () => {
    expect(
      ids(sortHoldings(FIXTURE_ROWS, { columnKey: "unrealized_pnl", order: "descend" })),
    ).toEqual(["h1", "h4", "h3", "h2"]);
  });
  ...
  it("多币种混合时金额列为数值直比（账户本位币不做汇率折算，已接受代价）", () => {
    ...
    expect(ids(sortHoldings(rows, { columnKey: "market_value", order: "ascend" }))).toEqual([
      "h1",
      "h4",
    ]);
    expect(ids(sortHoldings(rows, { columnKey: "market_value", order: "descend" }))).toEqual([
      "h4",
      "h1",
    ]);
  });
...
describe("holdingsSearchLabel / holdingMatchesSearch 匹配语义", () => {
  it("判定目标为「代码 · 名称」等价文本，无名称退化为裸代码，两者皆缺为空文本", () => {
    expect(holdingsSearchLabel({ symbol: "600000", instrumentName: "浦发银行" })).toBe(
      "600000 · 浦发银行",
    );
...
describe("filterHoldings 过滤语义", () => {
  it("账户过滤：仅保留所选账户行", () => {
    expect(ids(filterHoldings(FIXTURE_ROWS, { search: "", accountId: "acc-inv-2" }))).toEqual([
      "h3",
      "h4",
    ]);
  });
  ...
  it("搜索与账户过滤 AND 组合", () => {
    expect(ids(filterHoldings(FIXTURE_ROWS, { search: "txkg", accountId: "acc-inv-2" }))).toEqual([
      "h3",
    ]);
    expect(ids(filterHoldings(FIXTURE_ROWS, { search: "txkg", accountId: "acc-inv-1" }))).toEqual(
      [],
    );
  });
