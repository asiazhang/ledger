...
function groupMorePath(id: ClosableFeatureId): string {
  ...
  if (!record || typeof record.path !== "string")
    throw new Error(`路由表缺少 ${id} 所在组的「更多」页`);
  return record.path;
}
...
describe("关闭功能的裸路由守卫（issue #1244 / ADR-0116 决策 4/5）", () => {
  ...
  it.each([...CLOSABLE_FEATURES])(
    "打开态守卫放行（保单/保司按既有分流落资产·更多）",
    async (id) => {
  ...
  it("存量 /more 深链的页签指向已关闭功能：经迁移链也不落无页签可落的组页", async () => {
    ...
    expect((await landAt("/more?tab=policies&focus=pol-1")).currentRoute.value.name).toBe(
      "policies",
    );
  });
