...
/**
 * 基础布线表（overrides 优先于接缝参考兜底）：分类/商户为自定义夹具——商户解析与
 * 选项断言消费「订阅服务」「视频平台」；账户单行走账户选项断言；创建命令固定返回新 id。
 * 币种夹具与规范夹具一致，保司无断言消费，均走内建兜底。
 */
...
describe("useScheduledPlanForm 公共 payload 组装（CreateScheduledInput 公共字段单源）", () => {
  it("订阅形态：公共字段单源组装，备注 trim，不带形态特化键", async () => {
    ...
    expect(
      form.buildCreateInput({ kind: "subscription", amountCents: 2500, merchantId: "mch-1" }),
    ).toEqual({
      kind: "subscription",
      account_id: "acc-1",
      category_id: "cat-1",
...
describe("useScheduledPlanForm submitCreate 提交时序编排（spec #520）", () => {
  ...
  it("成功路径：创建命令 → 成功提示 → 公共草稿重置终态 → 提交成功后回调（回调见已重置草稿）", async () => {
    ...
    const form = useScheduledPlanForm({
      onSubmitted: () => {
        seen.push(`note=${form.note.value}`);
      },
    });
    // 先改成非初始值，再提交
    form.note.value = "月度储蓄";
  ...
  it("失败：错误提示（逐字维持 createFailed 文案）、不重置草稿、不触发提交成功后回调", async () => {
    ...
    expect(lastMessage("error")).toBe(
      "创建失败: 转出账户与转入账户币种不一致，定时转账不支持跨币种",
    );
    // 不重置草稿（弹窗保持打开）
