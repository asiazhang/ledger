...
describe('InvestmentsView 标的 tab', () => {
  ...
  it('标的 tab 分页请求携带 page/page_size', async () => {
    const wrapper = mountView()
    await nextTick()
    const instTab = wrapper.findAll('.n-tabs-tab')[2]
  ...
  it('离开走势 tab 清空入口标的：直入「走势」tab 回到默认组合曲线', async () => {
    ...
    await nextTick()
    await nextTick()
    await wrapper.findAll('.n-tabs-tab')[3].trigger('click')
    await flushPromises()
...
describe('InvestmentsView 持仓页签（issue #901）', () => {
  ...
  it('持仓页签完整呈现：按币种合计统计 + 持仓明细表 + 同步按钮在位', async () => {
    wireInvokeSeam({ defaults: { ...INVESTMENT_DEFAULTS, list_holdings: mockHoldings } })
    const wrapper = mountView()
    await flushPromises()
  ...
  it('盈亏页签不再渲染持仓卡（收窄为纯已实现盈亏视图），访问持仓页签后返回亦不残留', async () => {
    ...
    await clickTab(wrapper, 1)
    await clickTab(wrapper, 0)
    expect(wrapper.text()).not.toContain('当前持仓')
    expect(wrapper.find('[data-testid="sync-instrument-info"]').exists()).toBe(false)
  })

  it('同步实际写价后持仓列表与合计自动刷新：既有价格失效信号机制保持', async () => {
    wireInvokeSeam({ defaults: { ...INVESTMENT_DEFAULTS, list_holdings: mockHoldings } })
    const wrapper = mountView()
    await flushPromises()
    await clickTab(wrapper, 1)
