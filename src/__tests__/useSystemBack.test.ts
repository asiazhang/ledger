...
import {
  ...
  openOverlayNames,
  resetOverlays,
} from "@ledger/ui-kit/overlayRegistry";
    ...
    const Host = defineComponent({
      setup() {
        ...
        return () => [
          h(
            AppModal,
            { show: showModal.value, "onUpdate:show": (v: boolean) => (showModal.value = v) },
            { default: () => h("div", "modal-body") },
          ),
          ...
          h(
            ...
            {
              class: "open-dialog",
              onClick: () => dialog.warning({ title: "确认", content: "c" }),
            },
            "dialog",
          ),
        ];
