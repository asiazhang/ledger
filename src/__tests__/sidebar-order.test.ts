...
describe("parseGroupOrders × 收纳清单耦合（issue #474：收纳成员不回填主项，清单是成员资格唯一事实源）", () => {
  ...
  it("已存数组中的收纳成员按非法名过滤（不因「缺失补尾」复活为主项）", () => {
    expect(
      parseGroupOrders({ bookkeeping: ["transactions", "accounts", "budget"] }, contained)
        .bookkeeping,
...
describe("组内收纳清单：出厂种子与解析防御（issue #472/#473 / ADR-0063 决策 3/5）", () => {
  ...
  it("去重保留首现；缺失出厂成员按出厂序补尾（自定义成员保位，缺失者缀后）", () => {
    expect(parseContainmentLists({ bookkeeping: ["merchants", "merchants"] }).bookkeeping).toEqual([
      "merchants",
      "scheduled",
    ]);
    expect(parseContainmentLists({ assets: ["policies", "policies"] }).assets).toEqual([
      "policies",
      "physicalAssets",
      "insurers",
    ]);
    expect(parseContainmentLists({ bookkeeping: [] }).bookkeeping).toEqual([
      "scheduled",
      "merchants",
    ]);
...
describe("parseContainmentLists × 组内序耦合（issue #475：移回的出厂种子不复活回收纳清单）", () => {
  ...
  it("不带第二参数时行为不变（缺失种子照常补尾，既有调用零影响）", () => {
    expect(parseContainmentLists({ bookkeeping: ["merchants"] }).bookkeeping).toEqual([
      "merchants",
      "scheduled",
    ]);
  });
...
describe("parseGroupOrders × 移回种子（issue #475：移回的出厂种子是合法主项）", () => {
  ...
  it("组内序中的本组种子（未在清单）按主项保留原位，不按非法名过滤；缺失主项照常补尾", () => {
    expect(
      parseGroupOrders(
        { bookkeeping: ["budget", "scheduled", "accounts", "transactions"] },
        contained,
      ).bookkeeping,
  ...
  it("仍在清单中的种子依旧是非法名（清单是成员资格唯一事实源，#474 语义不变）；缺失主项照常补尾", () => {
    expect(
      parseGroupOrders(
        { bookkeeping: ["transactions", "scheduled"] },
        {
          ...contained,
          bookkeeping: ["scheduled", "merchants"],
        },
      ).bookkeeping,
    ).toEqual(["transactions", "accounts", "budget"]);
  });
});
...
describe("store 启动读路径（issue #269/#359/#472：读 view_state:* 两键，经解析防御后回默认）", () => {
  ...
  it("已存组内序对象：各组独立解析生效（缺失项补尾、非法名过滤），脏清单回种子", () => {
    ...
    localStorage.setItem(
      VIEW_STATE_KEYS.sidebarContainment,
      JSON.stringify({ assets: ["merchants"], insights: ["x"] }),
    );
    reboot();
    const store = useSidebarOrderStore();
  ...
  it("存储往返：saveContainmentLists 写入的清单经 parseContainmentLists 防御后还原", () => {
    ...
    expect(
      parseContainmentLists(JSON.parse(localStorage.getItem(VIEW_STATE_KEYS.sidebarContainment)!)),
    ).toEqual(lists);
  });
...
describe("buildSidebarSortMenuOptions（排序菜单选项构建纯函数，含组内边界置灰，issue #270/#359）", () => {
  ...
  it("菜单 key 与移动动作同一词表（防两套词表错位回归：菜单里的移动 key 必须全是合法 SidebarSortAction）", () => {
    const moveKeys = buildSidebarSortMenuOptions("budget", bookkeeping)
      ...
      .map((o) => o.key!)
      .filter(
        (k) => k !== "sort-divider" && k !== "reset-divider" && k !== "reset" && k !== "intoMore",
      );
  ...
  it("组内首位项：上移/移顶置灰，下移/移底可用", () => {
    const [up, down, top, bottom] = buildSidebarSortMenuOptions("transactions", bookkeeping).map(
      row,
    );
...
describe("右键「移入更多」写路径（issue #474：点选即追加本组清单尾、双存储立即持久化）", () => {
  ...
  it("applyMoveIntoMore：主项退出组内序 + 追加本组收纳清单尾，双存储立即持久化", () => {
    ...
    expect(store.sidebarContainment.bookkeeping).toEqual([
      "scheduled",
      "merchants",
      "transactions",
    ]);
    expect(JSON.parse(localStorage.getItem(ORDER_KEY)!)).toEqual({ ...store.sidebarGroupOrders });
    expect(JSON.parse(localStorage.getItem(CONTAINMENT_KEY)!)).toEqual({
      ...store.sidebarContainment,
    });
  });
  ...
  it("重启（新 pinia）后移入保持：主项不复活、清单保持（持久化往返）", () => {
    ...
    expect(rebooted.sidebarContainment.bookkeeping).toEqual([
      "scheduled",
      "merchants",
      "transactions",
    ]);
