...
describe("check-frontend-structure（前端 workspace 结构守门）", () => {
  ...
  describe("规则③：跨包引用形态（@ledger/* 包名，禁 @/ 与相对穿越）", () => {
    ...
    it("@/ 别名即红，定位文件与行号", () => {
      ...
      // 靶形是 @/ 别名本身，具体子路径任意
      const r = run(
        packageWithSource("import { helper } from '@/lib/format'\nexport { helper }\n"),
      );
      expect(r.status).toBe(1);
    ...
    it("相对路径穿越包边界即红", () => {
      const r = run(packageWithSource("import { x } from '../../other/src/y'\nexport { x }\n"));
      expect(r.status).toBe(1);
  ...
  describe("规则④：深导入禁令（跨包引用必须命中 exports 入口）", () => {
    ...
    function deepImportFixture(
      ...
      mkdirSync(join(root, "packages/a/src"), { recursive: true });
      writeFileSync(
        join(root, "packages/a/src/x.ts"),
        `import { y } from '${importSpec}'\nexport { y }\n`,
      );
      writePackageManifest(root, "packages/a", {
        name: "@ledger/a",
  ...
  describe("夹具登记表注入接缝（arg2）", () => {
    ...
    it("PACKAGES 生产登记表与已落位包全等（#1150/#1151/#1152/#1153/#1154/#1155/#1315/#1317/#1318/#1319/#1320/#1321/#1322/#1678 抽包落位；#1354 测试边同步）", () => {
