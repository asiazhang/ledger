...
describe("SettingsView.vue Tab 分域（issue #157 ADR-0022 立项；现役格局 7 页签）", () => {
  ...
  it("一键备份调用 create_backup 命令", async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        targetPath: expect.stringMatching(/ledger-backup-\d{8}-\d{6}\.db\.zip$/),
      }),
    );
  ...
  it("一键备份写入受管目录后自动滚动清理", async () => {
    ...
    await backupBtn.trigger("click");
    await flushPromises();
    expect(mockInvoke).toHaveBeenCalledWith("prune_backups", {
      dir: "/Users/me/backups",
      keep: 30,
    });
  });
  ...
  it("备份文件列表展示数量与上限，手动清理需确认", async () => {
    ...
    wireInvokeSeam({
      ...
      overrides: {
        ...
        prune_backups: () => ({
          kept: 1,
          deleted: ["ledger-backup-20260101-010101.db.zip"],
          failed: [],
        }),
      },
    });
    ...
    wireInvokeSeam({
      ...
      overrides: {
        ...
        get_data_location_info: () =>
          Promise.resolve(
            dataLocationInfo({ pending_restart: true, configured_dir: "/Users/me/ledger-data" }),
          ),
      },
    });
    const wrapper = mount(SettingsView);
...
describe("SettingsView.vue 定时 Tab：设备级自动执行开关（issue #308 / ADR-0042）", () => {
  ...
  it("切换开关更新 store 并持久化 localStorage（设备偏好落点，不经后端持久化）", async () => {
    ...
    expect(findCardByTitle(wrapper, "自动执行")!.find(".n-switch").attributes("aria-checked")).toBe(
      "true",
    );
    await sw.trigger("click");
