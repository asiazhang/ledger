...
describe("helpers/dom：按钮查找三变体 + findInput 家族（issue #746）", () => {
  ...
  it("findBodyButtonByTestId：body-teleport 弹窗按钮按 testid 查找", async () => {
    await mountProbe();
    expect((findBodyButtonByTestId("modal-ok")!.element as HTMLButtonElement).tagName).toBe(
      "BUTTON",
    );
  ...
  it("findInput 家族：裸 input / placeholder / type 三种形态", async () => {
    ...
    expect((findInput(wrapper, { type: "password" }).element as HTMLInputElement).type).toBe(
      "password",
    );
...
describe("helpers/dom：findInputByTestId（issue #748）", () => {
  it("经 testid 载体组件锚定输入框", async () => {
    ...
    expect((findInputByTestId(wrapper, "field-note").element as HTMLInputElement).type).toBe(
      "text",
    );
