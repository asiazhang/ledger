...
/**
 * 投资明细页签（Investment Ledger Tab，ADR-0135 决策 3 / issue #1779 基座、决策 4
 * 操作同权 / issue #1781）：五种投资 kind 交易行的投资投影列表——消费后端投资
 * 明细命令（issue #1778），按 kind 分形态渲染（buy/sell 标的/数量/单价/手续费/
 * 出资账户、convert「A → B」双腿、split 带符号份额增量 Δ、dividend 现金腿与到账
 * 账户）；服务端 offset 分页（ADR-0008，「共 N 条」+ 页大小档位与主列表同构）+
 * 类型多选筛选（投资 kind 子集）。
 *
